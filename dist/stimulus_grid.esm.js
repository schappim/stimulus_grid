...
var Ri = (t, s, e) => s in t ? Di(t, s, { enumerable: !0, configurable: !0, writable: !0, value: e }) : t[s] = e;
var V = (t, s, e) => Ri(t, typeof s != "symbol" ? s + "" : s, e);
import { Controller as ie, Application as Pi } from "@hotwired/stimulus";
function z(t, s) {
...
}, Hi = { text: rn, number: Ii, date: Vi, boolean: Fi, set: Bi };
function sn(t, s, e) {
  if (!e) return !0;
  const n = e.filterType || s.filter || "text", i = (Hi[n] || rn)[e.type];
  if (!i) return !0;
  const o = z(t, s);
...
function Oi(t, s, e) {
  ...
  const n = (l, c) => {
    for (const { colId: d, sort: p } of s) {
      const f = e[d];
      if (!f) continue;
      const g = z(l, f), h = z(c, f), y = typeof f.comparator == "function" ? f.comparator(g, h, l, c) : se(g, h, f.type);
...
function zi(t, s, e, n, r = () => !0) {
  const i = (c, d, p) => {
    const f = s[d], g = /* @__PURE__ */ new Map();
    for (const h of c) {
      const y = z(h, f), m = y == null ? "" : String(y);
      g.has(m) || g.set(m, { value: y, rows: [] }), g.get(m).rows.push(h);
    }
    return Array.from(g.values()).sort((h, y) => se(h.value, y.value, f.type)).map(({ value: h, rows: y }) => {
      ...
      return {
        ...
        aggregates: Pe(y, n, e),
        leaves: y,
        children: d + 1 < s.length ? i(y, d + 1, v) : null
      };
    });
  }, o = i(t, 0, ""), a = [], l = (c) => {
    for (const d of c)
...
function Ui(t, s, e) {
  ...
  for (const i of t)
    for (const o of s) {
      const a = cn(i, o, e), l = e.map((d) => i[d.field] == null ? "(Blank)" : String(i[d.field])).join(" · "), c = r ? l : `${l} · ${o.aggFunc}(${o.col.field})`;
      n.push({
        field: a,
        headerName: c,
        type: "number",
...
function Pt(t, s, e, n) {
  ...
  for (const o of s) {
    ...
    for (const c of e) {
      const d = cn(o, c, n);
      r[d] = l.length ? ln(c.aggFunc, l, c.col) : null;
    }
  }
  return r;
}
function qi({ rows: t, rowGroupCols: s = [], pivotCols: e, valueConfigs: n, isExpanded: r = () => !0, sortModel: i = [] }) {
  const o = ji(t, e), a = Ui(o, n, e), l = {
    __sgGroup: !0,
    __pivotAll: !0,
    level: -1,
    field: null,
  ...
  const c = (g, h, y) => {
    ...
    for (const C of g) {
      const S = z(C, m), A = S == null ? "" : String(S);
      v.has(A) || v.set(A, { value: S, rows: [] }), v.get(A).rows.push(C);
    }
    const b = Array.from(v.values()).map(({ value: C, rows: S }) => {
      const A = C == null ? "" : String(C), $ = y ? `${y}|${m.field}=${A}` : `${m.field}=${A}`;
      return {
        __sgGroup: !0,
        level: h,
        field: m.field,
        ...
        aggregates: {},
        leaves: S,
        __pivotValues: Pt(S, o, n, e),
        children: h + 1 < s.length ? c(S, h + 1, $) : null
      };
    }), w = Wi(i, m);
    return b.sort(w);
  }, d = c(t, 0, ""), p = [l], f = (g) => {
...
function Yi(t, { pivotCols: s = [], valueConfigs: e = [], columnGroups: n = null } = {}) {
...
function Xi(t, s = {}) {
  ...
  const e = t.map((i) => Yi(i, s).slice()), n = Math.max(1, ...e.map((i) => i.length)), r = [];
  for (let i = 0; i < n; i++) {
    const o = [];
    let a = 0;
    for (; a < e.length; ) {
      ...
      for (; d < e.length; ) {
        ...
        let f = !0;
        for (let g = 0; g < i; g++) {
          const h = l[g]?.id ?? null, y = p[g]?.id ?? null;
          if (h !== y) {
            f = !1;
            break;
          }
...
function Ji({
  ...
  for (const m of t) {
    const v = o(m), b = m?.[s], w = b == null ? null : String(b);
    w == null || w === v || !a.has(w) ? c.push(m) : (l.has(w) || l.set(w, []), l.get(w).push(m));
  }
  const d = n ? new Map(t.map((m) => [o(m), !!n(m)])) : null, p = /* @__PURE__ */ new Map(), f = (m, v) => {
    ...
    let w = !!d.get(b);
    const C = l.get(b) || [];
    for (const S of C) w = f(S, v) || w;
    return v.delete(b), p.set(b, w), w;
  };
  if (d)
    for (const m of c) f(m, /* @__PURE__ */ new Set());
  const g = [], h = /* @__PURE__ */ new Map(), y = (m, v, b, w) => {
    const C = d ? m.filter((S) => w || p.get(o(S))) : m.slice();
    r && C.sort(r);
    for (const S of C) {
      const A = o(S);
      if (A == null || b.has(A)) continue;
      const $ = l.get(A) || [], L = w || (d ? !!d.get(A) : !1), E = d ? $.filter((I) => L || p.get(o(I))) : $, N = E.length > 0, D = N && (d ? !0 : !!i(A, v));
      h.set(A, { level: v, hasChildren: N, expanded: D }), g.push(S), D && (b.add(A), y(E, v + 1, b, L), b.delete(A));
    }
  };
...
function Qi(t) {
  ...
  if (t.treeData && !t.pivotMode && n.length === 0) {
    const d = t.treeParentField || "parent_id", p = Object.entries(t.filterModel || {}).filter(([, S]) => S != null), f = t.quickFilter ? String(t.quickFilter).toLowerCase() : "", h = p.length > 0 || f !== "" ? (S) => {
      ...
      return !0;
    } : null, y = Array.isArray(t.sortModel) ? t.sortModel : [], m = y.length ? (S, A) => {
      for (const { colId: $, sort: L } of y) {
        const E = s[$];
        if (!E) continue;
        const N = z(S, E), D = z(A, E), I = typeof E.comparator == "function" ? E.comparator(N, D, S, A) : se(N, D, E.type);
        if (I !== 0) return L === "desc" ? -I : I;
      }
      return 0;
    } : null, v = t.getRowId || ((S) => S?.id), { displayList: b, treeMeta: w } = Ji({
      rows: t.rowData,
      parentField: d,
      getRowId: v,
      passesFilter: h,
  ...
  let r = t.rowData;
  r = on(r, t.filterModel, s), r = an(r, t.quickFilter, e), r = Oi(r, t.sortModel, s);
  ...
  if (t.pivotMode && o.length && a.length) {
    const d = i.map((v) => s[v]), p = o.map((v) => s[v]), { columns: f, displayList: g, tree: h, combos: y } = qi({
      rows: r,
      rowGroupCols: d,
      pivotCols: p,
      valueConfigs: a,
    ...
    return {
      ...
      tree: h,
      leafCount: r.length,
      grandTotals: Pe(r, t.aggModel, s),
      filteredSorted: g,
      ...m
    };
  }
  if (i.length) {
    const d = i.map((h) => s[h]), { displayList: p, tree: f } = zi(
      r,
      d,
      s,
      t.aggModel,
  ...
  const l = Le(r, t.pagination), c = t.aggModel && Object.keys(t.aggModel).length ? Pe(r, t.aggModel, s) : null;
  return { filteredSorted: r, grandTotals: c, ...l };
}
function eo(t, s, e, n, r = 6) {
...
const Ft = [
  ...
  20375,
  19104
], Ie = new Uint8Array(512), bt = new Uint8Array(256);
(function() {
  let s = 1;
  for (let e = 0; e < 255; e++)
    Ie[e] = s, bt[s] = e, s <<= 1, s & 256 && (s ^= 285);
  for (let e = 255; e < 512; e++) Ie[e] = Ie[e - 255];
})();
function yt(t, s) {
  return t === 0 || s === 0 ? 0 : Ie[bt[t] + bt[s]];
}
function so(t) {
...
function ao(t) {
  ...
  let e = 0;
  for (let L = 1; L <= 10; L++) {
    const N = 4 + (L < 10 ? 8 : 16) + s.length * 8, D = Ft[L - 1][0] * 8;
    if (N <= D) {
      e = L;
      break;
    }
  ...
  let y = 0;
  for (let L = 0; L < i; L++) {
    const E = L < i - f ? p : p + 1, N = c.slice(y, y + E);
    y += E, g.push({ data: N, ecc: io(N, h) });
  }
  const m = [], v = p + 1;
  for (let L = 0; L < v; L++)
    for (const E of g) L < E.data.length && m.push(E.data[L]);
  for (let L = 0; L < r; L++)
    for (const E of g) m.push(E.ecc[L]);
  const b = 17 + e * 4, w = new Uint8Array(b * b), C = new Uint8Array(b * b);
  lo(w, C, b), co(w, C, b), po(w, C, b, e), e >= 7 && fo(w, C, b, e), go(w, C, b, m);
  let S = 0, A = 1 / 0;
  const $ = new Uint8Array(w);
  for (let L = 0; L < 8; L++) {
    $.set(w), Ht($, C, b, L), Bt($, b, L);
    const E = ho($, b);
    E < A && (A = E, S = L);
  }
  return Ht(w, C, b, S), Bt(w, b, S), { size: b, matrix: w };
}
...
const un = '<svg viewBox="0 0 640 640" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path fill="currentColor" d="M471.1 297.4C483.6 309.9 483.6 330.2 471.1 342.7L279.1 534.7C266.6 547.2 246.3 547.2 233.8 534.7C221.3 522.2 221.3 501.9 233.8 489.4L403.2 320L233.9 150.6C221.4 138.1 221.4 117.8 233.9 105.3C246.4 92.8 266.7 92.8 279.2 105.3L471.1 297.4z"/></svg>', vt = /* @__PURE__ */ new Map();
function _(t, s) {
...
function ve(t) {
  return vt.get(t) || null;
}
function bo() {
  return Array.from(vt.keys());
}
function yo(t, { copy: s, parse: e } = {}) {
  return typeof s == "function" && (t.copyValue = s), typeof e == "function" && (t.parseValue = e), t;
}
const pn = /* @__PURE__ */ new Set(["1", "true", "t", "yes", "y", "on", "✓", "checked"]), fn = /* @__PURE__ */ new Set(["0", "false", "f", "no", "n", "off", "✗", "unchecked", "-", "—"]);
function wo(t, s) {
  const e = String(t ?? "");
  if (e === "") return "";
  switch (s?.type) {
    ...
    case "boolean": {
      ...
      return pn.has(n) ? !0 : fn.has(n) ? !1 : void 0;
    }
    case "date": {
      const n = new Date(e);
...
function vo(t) {
  ...
  if (s !== "") {
    if (pn.has(s)) return !0;
    if (fn.has(s)) return !1;
  }
}
...
const k = (t) => t == null || t === "", Co = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
function gn() {
  return ({ value: t }) => {
    if (k(t)) return "";
    const s = String(t);
    return Co.test(s) ? u("a", {
...
function mn({ defaultRegion: t = "AU" } = {}) {
  return ({ value: s }) => {
    if (k(s)) return "";
...
function bn({ currency: t = "USD", locale: s = "en-US", decimals: e } = {}) {
  return ({ value: n, td: r }) => {
...
function yn({ decimals: t = 0, scale: s = "as-is" } = {}) {
  return ({ value: e, td: n }) => {
...
function wn({ locale: t = void 0, dateStyle: s = "medium", ...e } = {}) {
...
function _n({ locale: t = void 0, dateStyle: s = "medium", timeStyle: e = "short", ...n } = {}) {
...
function vn({ locale: t = void 0, numeric: s = "auto", style: e = "long" } = {}) {
  const n = new Intl.RelativeTimeFormat(t, { numeric: s, style: e });
  return ({ value: r }) => {
    const i = j(r);
    if (!i) return "";
    const o = i.getTime() - Date.now(), a = Math.abs(o), l = ct.find((p) => a < p.cutoff) || ct[ct.length - 1], c = Math.round(o / l.ms), d = u("span", { class: "sg-renderer-relative-time", title: i.toLocaleString() });
    return d.textContent = n.format(c, l.unit), d;
  };
}
const xo = { ms: 1, sec: 1e3, second: 1e3, min: 6e4, minute: 6e4, hr: 36e5, hour: 36e5 };
function Cn({ unit: t = "ms", style: s = "compact" } = {}) {
  const e = xo[t] ?? 1;
  return ({ value: n, td: r }) => {
    if (r && r.classList.add("sg-renderer-number"), k(n)) return "";
    const i = Number(n) * e;
...
function xn({ locale: t = void 0, decimals: s, ...e } = {}) {
  const n = { ...e };
...
function Ln({ binary: t = !0, decimals: s = 1, locale: e = void 0 } = {}) {
...
const So = /* @__PURE__ */ new Set(["1", "true", "t", "yes", "y", "on"]);
function Ct(t) {
  return t === !0 || t === 1 ? !0 : t == null || t === "" || t === !1 || t === 0 ? !1 : So.has(String(t).toLowerCase());
}
const Lo = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335.1 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z"/></svg>', ko = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM175 175c9.4-9.4 24.6-9.4 33.9 0l47 47 47-47c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9l-47 47 47 47c9.4 9.4 9.4 24.6 0 33.9s-24.6 9.4-33.9 0l-47-47-47 47c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l47-47-47-47c-9.4-9.4-9.4-24.6 0-33.9z"/></svg>';
function kn({
  truthy: t = Ct,
  nullLabel: s = "—",
  falseStyle: e = "icon"
  ...
  return ({ value: n }) => {
    ...
    if (t(n)) {
      ...
      return i.innerHTML = Lo, i;
    }
    if (e === "hidden") return "";
    const r = u("span", { class: "sg-renderer-bool is-false", "aria-label": "false" });
    return r.innerHTML = ko, r;
  };
}
const Eo = '<svg viewBox="0 0 384 512" aria-hidden="true"><path fill="currentColor" d="M214.6 41.4c-12.5-12.5-32.8-12.5-45.3 0l-160 160c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L160 141.2 160 448c0 17.7 14.3 32 32 32s32-14.3 32-32l0-306.7L329.4 246.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3l-160-160z"/></svg>', $o = '<svg viewBox="0 0 384 512" aria-hidden="true"><path fill="currentColor" d="M169.4 470.6c12.5 12.5 32.8 12.5 45.3 0l160-160c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L224 370.7 224 64c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 306.7L54.6 265.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l160 160z"/></svg>', To = '<svg viewBox="0 0 448 512" aria-hidden="true"><path fill="currentColor" d="M64 256a32 32 0 1 0 0-64H384V160c0-12.9 7.8-24.6 19.8-29.6s25.7-2.2 34.9 6.9l96 96c12.5 12.5 12.5 32.8 0 45.3l-96 96c-9.2 9.2-22.9 11.9-34.9 6.9S384 364.9 384 352V320H64z"/></svg>';
function En({
  style: t = "percent",
  ...
  return t === "currency" ? o = new Intl.NumberFormat(e, {
    ...
    let d = "is-flat", p = To;
    const f = !r;
    c > 0 ? (d = f ? "is-up" : "is-down", p = Eo) : c < 0 && (d = f ? "is-down" : "is-up", p = $o);
    ...
    return g.append(h), g.append(u("span", { class: "sg-renderer-delta-value" }, document.createTextNode(y))), g;
  };
}
function $n({ chars: t = null } = {}) {
  return ({ value: s, td: e }) => {
    if (k(s)) return "";
    const n = String(s);
    let r = n, i = !1;
...
const tt = '<svg viewBox="0 0 448 512" aria-hidden="true"><path fill="currentColor" d="M384 336H192c-8.8 0-16-7.2-16-16V64c0-8.8 7.2-16 16-16h140.1L400 115.9V320c0 8.8-7.2 16-16 16zM192 384H384c35.3 0 64-28.7 64-64V115.9c0-12.7-5.1-24.9-14.1-33.9L366.1 14.1c-9-9-21.2-14.1-33.9-14.1H192c-35.3 0-64 28.7-64 64V320c0 35.3 28.7 64 64 64zM64 128c-35.3 0-64 28.7-64 64V448c0 35.3 28.7 64 64 64H256c35.3 0 64-28.7 64-64V416H272v32c0 8.8-7.2 16-16 16H64c-8.8 0-16-7.2-16-16V192c0-8.8 7.2-16 16-16H96V128H64z"/></svg>', Tn = '<svg viewBox="0 0 448 512" aria-hidden="true"><path fill="currentColor" d="M438.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L160 338.7 393.4 105.4c12.5-12.5 32.8-12.5 45.3 0z"/></svg>';
function An({ position: t = "after" } = {}) {
  return ({ value: s }) => {
    if (k(s)) return "";
    ...
    return i.innerHTML = tt, i.addEventListener("click", async (o) => {
      o.stopPropagation(), o.preventDefault();
      try {
        navigator.clipboard?.writeText ? await navigator.clipboard.writeText(e) : Nn(e), i.innerHTML = Tn, i.classList.add("is-copied"), setTimeout(() => {
          i.innerHTML = tt, i.classList.remove("is-copied");
        }, 1200);
      } catch {
      }
...
function Rn({
  ...
  return ({ value: a }) => {
    ...
    const c = r ?? Math.min(...l), p = Math.max(...l, r ?? -1 / 0) - c || 1, f = 1.5, g = 2.5, h = s - f * 2, y = e - g * 2, m = (w) => f + (l.length === 1 ? h / 2 : w / (l.length - 1) * h), v = (w) => g + y - (w - c) / p * y;
    let b = "";
    if (t === "bar") {
      const C = Math.max(1, (h - (l.length - 1) * 1) / l.length);
      for (let S = 0; S < l.length; S++) {
        const A = l[S], $ = f + S * (C + 1), L = v(A), E = g + y - L;
        b += `<rect x="${$.toFixed(2)}" y="${L.toFixed(2)}" width="${C.toFixed(2)}" height="${E.toFixed(2)}" fill="${o}"/>`;
      }
    } else {
      let w = "";
      for (let C = 0; C < l.length; C++)
        w += `${C === 0 ? "M" : "L"} ${m(C).toFixed(2)} ${v(l[C]).toFixed(2)} `;
      if (t === "area") {
        const C = w + ` L ${m(l.length - 1).toFixed(2)} ${(g + y).toFixed(2)} L ${m(0).toFixed(2)} ${(g + y).toFixed(2)} Z`;
      ...
      if (b += `<path d="${w.trim()}" fill="none" stroke="${o}" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>`, i) {
        const C = m(l.length - 1), S = v(l[l.length - 1]);
...
function Vn({
  ...
  const o = e.map(Pn).filter(Boolean);
  ...
  return ({ value: a, td: l }) => {
    ...
    const p = d * (o.length - 1), f = Math.min(o.length - 2, Math.floor(p)), g = p - f, h = Mo(o[f], o[f + 1], g);
    return l && (l.style.backgroundColor = No(...h), l.style.color = In(h) ? "#111827" : "#ffffff"), r ? typeof i == "function" ? i(a) : String(a) : "";
  };
}
const Do = {
  ...
  "cc-last4": (t, s) => Ot(t.replace(/\D/g, ""), 4, 4, s, " "),
  // BIN + last4: show the first 6 + last 4, mask the middle (PCI-friendly).
  "cc-bin-last4": (t, s) => Ot(t.replace(/\D/g, ""), 4, 4, s, " ", 6),
...
const Po = /* @__PURE__ */ new Set(["cc-last4", "cc-bin-last4", "phone-last4", "last4"]);
function Fn({
  format: t = null,
  showFirst: s = 0,
  showLast: e = 4,
  ...
  const i = t ? Do[t] : null, o = t ? Po.has(t) : !1, a = r === "right" || r !== "left" && o;
  return ({ value: l, td: c }) => {
...
function Bn({
  ...
  return ({ value: n, api: r }) => {
    ...
    return o ? Io(i, o, s, e) : document.createTextNode(i);
  };
}
function Io(t, s, e, n) {
...
const Vo = /* @__PURE__ */ new Set(["png", "jpg", "jpeg", "gif", "webp", "avif", "svg", "bmp", "ico"]);
function re(t) {
  if (!t) return !1;
...
}, On = '<svg viewBox="0 0 448 512" aria-hidden="true"><path fill="currentColor" d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z"/></svg>', St = '<svg viewBox="0 0 384 512" aria-hidden="true"><path fill="currentColor" d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"/></svg>', Fo = '<svg viewBox="0 0 320 512" aria-hidden="true"><path fill="currentColor" d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L77.3 256 214.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160z"/></svg>', Bo = '<svg viewBox="0 0 320 512" aria-hidden="true"><path fill="currentColor" d="M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-160 160c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 105.4 118.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l160 160z"/></svg>', Ho = /* @__PURE__ */ new Set(["mp3", "wav", "flac", "m4a", "ogg", "aac", "opus"]), Oo = /* @__PURE__ */ new Set(["mp4", "mov", "webm", "mkv", "avi", "m4v"]);
function Gn(t) {
  const s = String(t?.content_type || "").toLowerCase(), e = String(t?.filename || "").split(".").pop()?.toLowerCase() || "";
  return s.includes("pdf") || e === "pdf" ? "pdf" : s.startsWith("audio/") || Ho.has(e) ? "audio" : s.startsWith("video/") || Oo.has(e) ? "video" : s.includes("zip") || ["zip", "tar", "gz", "7z", "rar"].includes(e) ? "zip" : s.includes("sheet") || s.includes("excel") || s.includes("csv") || ["xls", "xlsx", "csv", "numbers"].includes(e) ? "sheet" : s.includes("word") || s.includes("document") || ["doc", "docx", "txt", "md", "rtf"].includes(e) ? "doc" : ["js", "ts", "rb", "py", "go", "rs", "java", "json", "xml", "html", "css", "sh", "sql"].includes(e) ? "code" : "file";
}
function ot(t) {
  if (t == null || t === "") return [];
  let s = t;
...
function zn({
  ...
  return (c) => {
    const { value: d, td: p, row: f, col: g } = c, h = ot(d);
    ...
    const y = u("div", { class: "sg-renderer-attachments", role: "group" }), m = h.slice(0, s), v = Math.max(0, h.length - m.length);
    if (m.forEach((b) => y.append(Go(b, t, h, o))), v > 0) {
      const b = u(
        "span",
      ...
      b.addEventListener("click", (w) => {
        w.stopPropagation(), jn(h, h[m.length]);
      }), y.append(b);
    }
    if (n) {
      const b = u("button", {
      ...
      b.innerHTML = On, b.addEventListener("click", (w) => {
        w.stopPropagation(), Gt(p, c, { thumbSize: t, accept: r, multiple: i, onUpload: a, onRemove: l });
      }), y.append(b), zo(p, c, { onUpload: a }), p.addEventListener("dblclick", (w) => {
        w._sgAttachmentHandled || (w._sgAttachmentHandled = !0, w.stopPropagation(), Gt(p, c, { thumbSize: t, accept: r, multiple: i, onUpload: a, onRemove: l }));
      }, { once: !1 });
    }
    return y;
  };
...
function Go(t, s, e, n) {
  const r = u("button", {
    ...
    title: `${t.filename}${t.byte_size != null ? " · " + Se(t.byte_size) : ""}`,
  ...
  if (re(t) && t.thumb_url)
    ...
    const i = Gn(t), o = u("span", { class: `sg-attach-icon is-${i}`, "aria-hidden": "true" });
    o.innerHTML = nt[i] || nt.file, r.append(o);
  }
...
function jn(t, s) {
  ...
  l.innerHTML = Fo, c.innerHTML = Bo;
  function d() {
    const g = e[n];
    o.src = g.preview_url || g.url, o.alt = g.filename, a.textContent = `${g.filename}${g.byte_size != null ? " · " + Se(g.byte_size) : ""} (${n + 1}/${e.length})`, l.style.visibility = e.length > 1 ? "visible" : "hidden", c.style.visibility = e.length > 1 ? "visible" : "hidden";
  }
  function p(g) {
    n = (n + g + e.length) % e.length, d();
  }
  function f(g) {
    g.key === "Escape" ? dt() : g.key === "ArrowLeft" ? p(-1) : g.key === "ArrowRight" && p(1);
  }
  r.addEventListener("click", (g) => {
    (g.target === r || g.target === i) && dt();
  }), l.addEventListener("click", (g) => {
    g.stopPropagation(), p(-1);
  }), c.addEventListener("click", (g) => {
    g.stopPropagation(), p(1);
  }), document.addEventListener("keydown", f), i.append(l, o, c), r.append(i, a), document.body.appendChild(r), ye = { overlay: r, onKey: f }, d();
}
function dt() {
  ye && (document.removeEventListener("keydown", ye.onKey), ye.overlay.remove(), ye = null);
}
let Ve = null;
function zo(t, s, { onUpload: e }) {
...
function Gt(t, s, e) {
  ke();
  const { thumbSize: n, accept: r, multiple: i, onUpload: o, onRemove: a } = e, l = t._sgAttachments || ot(s.value), c = u("div", { class: "sg-attach-editor", role: "dialog", "aria-modal": "false" });
  ...
  const d = u("div", { class: "sg-attach-editor-header" }, [
    ...
    (() => {
      ...
      return b.innerHTML = St, b.addEventListener("click", ke), b;
    })()
  ]), p = u("div", { class: "sg-attach-editor-grid" });
  function f() {
    const b = t._sgAttachments || [];
    p.replaceChildren(), b.forEach((w) => p.append(jo(w, t, s, a, n))), d.firstChild.textContent = b.length === 1 ? "1 attachment" : `${b.length} attachments`;
  }
  f(), t._sgAttachRepaint = f;
  const g = u("label", { class: "sg-attach-dropzone", tabindex: "0" });
  g.innerHTML = `
    <span class="sg-attach-dropzone-icon" aria-hidden="true">${On}</span>
  ...
  h.style.display = "none", g.append(h), h.addEventListener("change", async () => {
    h.files?.length && (await Fe(t, s, Array.from(h.files), o), h.value = "", f());
    ...
    b.dataTransfer?.files?.length && (b.preventDefault(), g.classList.remove("is-drop-target"), await Fe(t, s, Array.from(b.dataTransfer.files), o), f());
  });
  function y(b) {
    const w = Array.from(b.clipboardData?.files || []);
    w.length !== 0 && (b.preventDefault(), Fe(t, s, w, o).then(f));
  }
  ...
  function v(b) {
    !c.contains(b.target) && !t.contains(b.target) && ke();
  }
  document.addEventListener("keydown", m), setTimeout(() => document.addEventListener("mousedown", v), 0), c.append(d, p, g), document.body.appendChild(c), U(c, t), g.focus(), Ve = { pop: c, onKey: m, onDocClick: v, anchor: t };
}
function ke() {
  if (!Ve) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = Ve;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), n && delete n._sgAttachRepaint, Ve = null;
}
function jo(t, s, e, n, r) {
  ...
  if (re(t) && t.thumb_url)
    ...
    const c = Gn(t), d = u("span", { class: `sg-attach-icon is-${c}`, "aria-hidden": "true" });
    d.innerHTML = nt[c] || nt.file, o.append(d);
  }
  const a = u("div", { class: "sg-attach-editor-meta" }, [
    ...
    u(
      ...
      document.createTextNode(t.byte_size != null ? Se(t.byte_size) : "")
    )
  ]), l = u("button", {
    type: "button",
  ...
  return l.innerHTML = St, l.addEventListener("click", async (c) => {
    c.stopPropagation(), await Uo(s, e, t, n);
  }), i.append(o, a, l), i;
}
function U(t, s) {
  ...
  if (e.length) {
    ...
    try {
      ...
      if (typeof n == "function") {
        const i = await n(e, s);
        r = Array.isArray(i) ? i : (t._sgAttachments || []).concat(zt(e));
      } else
        r = (t._sgAttachments || []).concat(zt(e));
      Un(t, s, ot(r));
    } finally {
...
const Kn = ["NSW", "VIC", "QLD", "WA", "SA", "TAS", "ACT", "NT"], Lt = {
  NSW: "New South Wales",
  VIC: "Victoria",
  QLD: "Queensland",
...
function Wn({ editable: t = !0, empty: s = "" } = {}) {
  return (e) => {
    const { value: n, td: r } = e, i = Ko(n);
    ...
    t && r && !r._sgAddressEditBound && (r._sgAddressEditBound = !0, r.addEventListener("dblclick", (c) => {
      c._sgAddressHandled || (c._sgAddressHandled = !0, c.stopPropagation(), qo(r, e));
    }));
    const o = u("div", {
...
function qo(t, s) {
  ...
  n.addEventListener("mousedown", (N) => N.stopPropagation());
  ...
  function o({ label: N, name: D, type: I = "text", value: B = "", maxlength: H, inputmode: F, placeholder: X, autocomplete: ee }) {
    const Z = u("label", { class: "sg-address-au-editor-field", "data-field": D });
    Z.append(u("span", { class: "sg-address-au-editor-label" }, document.createTextNode(N)));
    const te = u("input", {
      type: I,
      name: D,
      value: B || "",
    ...
    return Z.append(te), { wrap: Z, input: te };
  }
  const a = o({
  ...
  function f() {
    const N = !!(l.input.value.trim() || d.input.value.trim());
    c.hidden = !N, p.hidden = N;
  }
  ...
  for (const N of Kn) {
    const D = u(
      "option",
      { value: N, selected: e.state === N ? "" : null },
      document.createTextNode(`${N} — ${Lt[N]}`)
    );
    y.append(D);
  }
  h.append(y);
  ...
  }), b = u("div", { class: "sg-address-au-editor-grid" });
  b.append(a.wrap), b.append(l.wrap, p), b.append(c), b.append(g.wrap, h, m.wrap), b.append(v.wrap);
  ...
  w.append(C, S), i.append(b, w), n.append(r, i);
  function A() {
    return {
  ...
  function $() {
    const N = A(), D = !N.address1 && !N.suburb && !N.state && !N.postcode;
    Yo(t, s, D ? null : N), fe();
  }
  ...
  function L(N) {
    N.key === "Escape" && (N.stopPropagation(), fe());
  }
  function E(N) {
    !n.contains(N.target) && !t.contains(N.target) && fe();
  }
  document.addEventListener("keydown", L), setTimeout(() => document.addEventListener("mousedown", E), 0), document.body.appendChild(n), U(n, t), f(), a.input.focus(), a.input.select(), Be = { pop: n, onKey: L, onDocClick: E };
}
function fe() {
  if (!Be) return;
  const { pop: t, onKey: s, onDocClick: e } = Be;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), Be = null;
}
function Yo(t, s, e) {
...
function qn({ color: t = "green", showValue: s = !1 } = {}) {
  return ({ value: e }) => {
    let n = Number(e);
...
const we = '<svg viewBox="0 0 576 512" aria-hidden="true"><path fill="currentColor" d="M316.9 18C311.6 7 300.4 0 288.1 0s-23.4 7-28.8 18L195 150.3 51.4 171.5c-12 1.8-22 10.2-25.7 21.7s-.7 24.2 7.9 32.7L137.8 329 113.2 474.7c-2 12 3 24.2 12.9 31.3s23 8 33.8 2.3l128.3-68.5 128.3 68.5c10.8 5.7 23.9 4.9 33.8-2.3s14.9-19.3 12.9-31.3L438.5 329 542.7 225.9c8.6-8.5 11.7-21.2 7.9-32.7s-13.7-19.9-25.7-21.7L381.2 150.3 316.9 18z"/></svg>';
function Yn({ max: t = 5, precision: s = 0.5 } = {}) {
  const e = s > 0 ? 1 / s : 2;
  return ({ value: n }) => {
    let r = parseFloat(n);
    ...
    for (let o = 1; o <= t; o++)
      if (r >= o)
        i.append(u("span", { class: "sg-renderer-star is-full", "aria-hidden": "true" }, we));
      else if (r > o - 1) {
        const a = Math.round((r - (o - 1)) * 100);
        i.append(u(
          "span",
          { class: "sg-renderer-star is-partial", "aria-hidden": "true" },
          `${we}<span class="sg-star-clip" style="width: ${a}%;">${we}</span>`
        ));
      } else
        i.append(u("span", { class: "sg-renderer-star is-empty", "aria-hidden": "true" }, we));
    return i;
  };
}
function Zn({ separator: t = "," } = {}) {
  return ({ value: s }) => {
    if (k(s)) return "";
...
function Xn({ showCode: t = !0, fallback: s = null } = {}) {
  return ({ value: e }) => {
    if (k(e)) return "";
...
function ea(t = {}, s = null, e = {}) {
  ...
  return ({ value: a }) => {
    if (k(a)) return "";
    const l = String(a).toLowerCase(), c = i[l] || r, d = n ? Qo(a) : String(a), p = u("span", { class: `sg-pill sg-pill-${c}` });
    if (s) {
      const f = o[l], g = f ? Jo[f] || f : null;
      if (g) {
...
const ta = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M464 256A208 208 0 1 0 48 256a208 208 0 1 0 416 0zM0 256a256 256 0 1 1 512 0A256 256 0 1 1 0 256zM188.3 147.1c7.6-4.2 16.8-4.1 24.3 .5l144 88c7.1 4.4 11.5 12.1 11.5 20.5s-4.4 16.1-11.5 20.5l-144 88c-7.4 4.5-16.7 4.7-24.3 .5s-12.3-12.2-12.3-20.9V168c0-8.7 4.7-16.7 12.3-20.9z"/></svg>', ut = '<svg viewBox="0 0 384 512" aria-hidden="true"><path fill="currentColor" d="M73 39c-14.8-9.1-33.4-9.4-48.5-.9S0 62.6 0 80V432c0 17.4 9.4 33.4 24.5 41.9s33.7 8.1 48.5-.9L361 297c14.3-8.7 23-24.2 23-41s-8.7-32.2-23-41L73 39z"/></svg>', na = '<svg viewBox="0 0 320 512" aria-hidden="true"><path fill="currentColor" d="M48 64C21.5 64 0 85.5 0 112V400c0 26.5 21.5 48 48 48H80c26.5 0 48-21.5 48-48V112c0-26.5-21.5-48-48-48H48zm192 0c-26.5 0-48 21.5-48 48V400c0 26.5 21.5 48 48 48h32c26.5 0 48-21.5 48-48V112c0-26.5-21.5-48-48-48H240z"/></svg>', ra = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M125.7 160H176c17.7 0 32 14.3 32 32s-14.3 32-32 32H48c-17.7 0-32-14.3-32-32V64c0-17.7 14.3-32 32-32s32 14.3 32 32v51.2L97.6 97.6c87.5-87.5 229.3-87.5 316.8 0s87.5 229.3 0 316.8s-229.3 87.5-316.8 0c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0c62.5 62.5 163.8 62.5 226.3 0s62.5-163.8 0-226.3s-163.8-62.5-226.3 0L125.7 160z"/></svg>', sa = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M386.3 160H336c-17.7 0-32 14.3-32 32s14.3 32 32 32H464c17.7 0 32-14.3 32-32V64c0-17.7-14.3-32-32-32s-32 14.3-32 32v51.2L414.4 97.6c-87.5-87.5-229.3-87.5-316.8 0s-87.5 229.3 0 316.8s229.3 87.5 316.8 0c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0c-62.5 62.5-163.8 62.5-226.3 0s-62.5-163.8 0-226.3s163.8-62.5 226.3 0L386.3 160z"/></svg>', ia = St;
function tr(t) {
  if (t == null || t === "") return null;
  if (typeof t == "string") {
    const e = t.trim();
...
function nr({
  ...
  return (i) => {
    ...
    a && !a._sgAudioDblBound && (a._sgAudioDblBound = !0, a.addEventListener("dblclick", (p) => {
      p._sgAudioHandled || (p._sgAudioHandled = !0, p.stopPropagation(), p.preventDefault(), jt(a, i));
    }));
    const c = u("div", { class: "sg-renderer-audio" }), d = u("button", {
      type: "button",
      class: "sg-audio-icon",
      title: `${l.filename}${l.byte_size != null ? " · " + Se(l.byte_size) : ""} — double-click to play`,
      "aria-label": `Play ${l.filename}`,
      "data-sg-audio": "open"
    });
    if (d.innerHTML = ta, d.addEventListener("click", (p) => {
      p.stopPropagation(), jt(a, i);
...
function oa(t, { preferHowler: s } = {}) {
  return s && typeof window < "u" && window.Howl ? new la(t) : new aa(t);
}
class aa {
  constructor(s) {
...
function jt(t, s) {
  ...
  e.byte_size != null && c.push(Se(e.byte_size)), r.backendName() === "howler" && c.push("howler.js"), l.textContent = c.join(" · ");
  const d = u("button", {
    type: "button",
    class: "sg-audio-player-close",
    "aria-label": "Close player"
  });
  d.innerHTML = ia, d.addEventListener("click", Ee), o.append(a, l, d);
  const p = u("div", {
  ...
  p.append(f, g);
  const h = u("div", { class: "sg-audio-times" }), y = u("span", { class: "sg-audio-time-current" }, document.createTextNode("0:00")), m = u(
    "span",
    { class: "sg-audio-time-total" },
    document.createTextNode(e.duration ? _e(e.duration) : "--:--")
  );
  h.append(y, m);
  const v = u("div", { class: "sg-audio-transport" }), b = u("button", {
    type: "button",
  ...
  C.innerHTML = sa, v.append(b, w, C), i.append(o, p, h, v);
  let S = e.duration ?? 0, A = !1, $ = null;
  function L(R) {
  ...
  function E() {
    ...
    if (Q > 0 && Q !== S && (S = Q, m.textContent = _e(S), p.setAttribute("aria-valuemax", String(Math.floor(S)))), !A) {
      const J = S > 0 ? R / S * 100 : 0;
      L(J), y.textContent = _e(R), p.setAttribute("aria-valuenow", String(Math.floor(R)));
    }
  }
  function N() {
    E(), r.isPlaying() ? $ = requestAnimationFrame(N) : $ = null;
  }
  function D() {
    $ == null && ($ = requestAnimationFrame(N));
  }
  function I() {
    $ != null && cancelAnimationFrame($), $ = null;
  }
  const B = () => {
    S = r.duration(), E();
  }, H = () => {
    w.dataset.state = "playing", w.innerHTML = na, w.setAttribute("aria-label", "Pause"), D();
  }, F = () => {
    w.dataset.state = "paused", w.innerHTML = ut, w.setAttribute("aria-label", "Play"), I(), E();
  }, X = () => {
    w.dataset.state = "paused", w.innerHTML = ut, w.setAttribute("aria-label", "Play"), I(), r.seek(0), E();
  };
  r.on("load", B), r.on("play", H), r.on("pause", F), r.on("end", X), w.addEventListener("click", (R) => {
    ...
    R.stopPropagation();
    const G = r.duration();
    r.seek(Math.min(G || 1 / 0, r.seek() + n.skipSeconds)), E();
  });
  function ee(R) {
    const G = p.getBoundingClientRect(), Q = (R.clientX ?? 0) - G.left, J = Math.max(0, Math.min(1, Q / G.width)), Dt = r.duration() || S;
    if (!Dt) return;
    const Rt = J * Dt;
    r.seek(Rt), L(J * 100), y.textContent = _e(Rt);
  }
  p.addEventListener("pointerdown", (R) => {
    R.preventDefault(), A = !0, p.setPointerCapture?.(R.pointerId), p.classList.add("is-dragging"), ee(R);
  ...
  p.addEventListener("pointerup", Z), p.addEventListener("pointercancel", Z), p.addEventListener("keydown", (R) => {
    const G = r.duration() || S;
    if (!G) return;
    const Q = R.shiftKey ? 30 : 5;
    let J = null;
    R.key === "ArrowLeft" ? J = Math.max(0, r.seek() - Q) : R.key === "ArrowRight" ? J = Math.min(G, r.seek() + Q) : R.key === "Home" ? J = 0 : R.key === "End" && (J = G), J != null && (R.preventDefault(), r.seek(J), E());
  });
  function te(R) {
    R.key === "Escape" ? (R.preventDefault(), Ee()) : (R.key === " " || R.code === "Space") && i.contains(document.activeElement) && (R.preventDefault(), r.isPlaying() ? r.pause() : r.play());
  }
  function O(R) {
    !i.contains(R.target) && !t.contains(R.target) && Ee();
  }
  document.addEventListener("keydown", te), setTimeout(() => document.addEventListener("mousedown", O), 0), document.body.appendChild(i), U(i, t), E(), w.focus(), He = {
    pop: i,
    backend: r,
    onKey: te,
    onDocClick: O,
    cleanup: () => {
      I();
      try {
        r.off("load", B), r.off("play", H), r.off("pause", F), r.off("end", X);
      } catch {
      }
      r.destroy();
    }
...
function Ee() {
  if (!He) return;
  const { pop: t, onKey: s, onDocClick: e, cleanup: n } = He;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), n(), t.remove(), He = null;
}
function rr({
  truthy: t = Ct,
  disabled: s = !1
...
`), e = [];
  let n = null, r = [];
  const i = () => {
    n && (e.push(`<${n}>${r.map((o) => `<li>${wt(o)}</li>`).join("")}</${n}>`), n = null, r = []);
  };
  for (const o of s) {
    const a = /^\s*[-*]\s+(.+)$/.exec(o), l = /^\s*\d+\.\s+(.+)$/.exec(o);
    a ? (n && n !== "ul" && i(), n = "ul", r.push(a[1])) : l ? (n && n !== "ol" && i(), n = "ol", r.push(l[1])) : (i(), o.trim() === "" ? e.push("") : e.push(wt(o)));
  }
  return i(), e.join("<br>").replace(/(<br>){2,}/g, "<br><br>");
}
function sr({ inline: t = !1 } = {}) {
  return ({ value: s, td: e }) => {
    if (k(s)) return "";
    const n = xe(s), r = t ? wt(n) : da(n);
    if (e) {
...
function ir({ maxKeys: t = 3, indent: s = 2 } = {}) {
  return ({ value: e, td: n }) => {
    if (e == null || e === "") return "";
    let r = e;
    if (typeof e == "string")
    ...
    a.innerHTML = un, o.append(a), o.append(u(
      "span",
      { class: "sg-renderer-json-summary-text" },
      document.createTextNode(pa(r, t))
    ));
    const l = document.createElement("pre");
    if (l.className = "sg-renderer-json-pre", l.innerHTML = ua(JSON.stringify(r, null, s)), i.append(o, l), o.addEventListener("click", (c) => c.stopPropagation()), n) {
...
function or({
  ...
  return ({ value: o, row: a }) => {
    ...
    for (const d of l) {
      const p = fa(d, a, t, s);
      c.append(ga(d, a, p, { showThumb: e, href: n, fallback: i }));
    }
    return c;
  };
}
...
function ga(t, s, e, { showThumb: n, href: r, fallback: i }) {
...
function ar({
  ...
  return ({ value: r }) => {
    ...
    for (const a of i) {
      ...
      /^(gray|red|orange|yellow|green|blue|indigo|purple|pink)$/.test(c) ? d.classList.add(`sg-pill-${c}`) : (d.style.background = c, d.style.color = lr(c)), o.append(d);
    }
    return o;
  };
}
function lr(t) {
  const s = Pn(t);
  return s ? In(s) ? "#1f2937" : "#ffffff" : "inherit";
}
function kt(t) {
  if (t == null || t === "") return null;
...
function dr({
  ...
  return ({ value: n }) => {
    if (k(n)) return "";
    const { from: r, to: i } = ha(n), o = (l) => l == null || l === "";
    if (o(r) && o(i)) return "";
    if (o(r))
      return u(
        "span",
...
function ur({
  ...
  return ({ value: o }) => {
    ...
    const c = s === "dms" ? `${Ut(a.lat, !0)} ${Ut(a.lng, !1)}` : `${a.lat.toFixed(t)}, ${a.lng.toFixed(t)}`;
...
function pr({
  ...
  return ({ value: i }) => {
    ...
    try {
      const c = ao(o);
      a = mo(c, { moduleSize: t, margin: s, background: e, foreground: n });
    } catch {
      return u(
        "span",
...
function fr({
  ...
  return ({ value: e, td: n }) => {
    ...
    if (t && i.append(u(
      ...
      a.innerHTML = tt, a.addEventListener("click", async (l) => {
        l.stopPropagation();
        try {
          navigator.clipboard?.writeText ? await navigator.clipboard.writeText(r) : Nn(r), a.innerHTML = Tn, a.classList.add("is-copied"), setTimeout(() => {
            a.innerHTML = tt, a.classList.remove("is-copied");
          }, 1200);
        } catch {
        }
      }), i.append(a);
...
const ba = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M225.8 468.2L46.9 295.4C18.9 268.2 0 230.1 0 187.8C0 117.4 56.1 60 125.7 60c25.3 0 51 9.2 70.3 25.4L256 138l60-52.6C335.3 69.2 361 60 386.3 60C455.9 60 512 117.4 512 187.8c0 42.3-18.9 80.4-46.9 107.6L286.2 468.2c-7.2 7.4-17.1 11.8-27.6 11.8c-10.5 0-20.4-4.4-27.6-11.8z"/></svg>', ya = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M313.4 32.9c26 5.2 42.9 30.5 37.7 56.5l-2.3 11.4c-5.3 26.7-15.1 52.1-28.8 75.2H464c26.5 0 48 21.5 48 48c0 18.5-10.5 34.6-25.9 42.6C497 275.4 504 288.9 504 304c0 23.4-16.8 42.9-38.9 47.1c4.4 7.3 6.9 15.8 6.9 24.9c0 21.3-13.9 39.4-33.1 45.6c.7 3.3 1.1 6.8 1.1 10.4c0 26.5-21.5 48-48 48H294.5c-19 0-37.5-5.6-53.3-16.1l-38.5-25.7C176 420.4 160 390.4 160 358.3V320 272 247.1c0-29.2 13.3-56.7 36-75.1c16.3-13.2 28.9-30.4 36.6-50l8.1-20.3c5-12.4 11.3-24.2 19-35.2l4.4-6.3 0 0c1.4-2 1.1-4.7-.6-6.4l0 0c-3.8-3.8-9.9-3.8-13.7 0L208.8 84.9c-3 3-7 4.7-11.3 4.7c-8.8 0-16-7.2-16-16V63.4c0-8.8 7.2-16 16-16h.5c8.8 0 16 7.2 16 16v6.2c34-2.4 71-13 100-25.6c11-4.8 22.1-9.3 33-13.6l1-.4c12-4.6 27.3-9.3 41.4-9.3z"/></svg>', wa = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M313.4 479.1c26-5.2 42.9-30.5 37.7-56.5l-2.3-11.4c-5.3-26.7-15.1-52.1-28.8-75.2H464c26.5 0 48-21.5 48-48c0-18.5-10.5-34.6-25.9-42.6C497 236.6 504 223.1 504 208c0-23.4-16.8-42.9-38.9-47.1c4.4-7.3 6.9-15.8 6.9-24.9c0-21.3-13.9-39.4-33.1-45.6c.7-3.3 1.1-6.8 1.1-10.4c0-26.5-21.5-48-48-48H294.5c-19 0-37.5 5.6-53.3 16.1L202.7 73.8C176 91.6 160 121.6 160 153.7V192v48 24.9c0 29.2 13.3 56.7 36 75.1c16.3 13.2 28.9 30.4 36.6 50l8.1 20.3c5 12.4 11.3 24.2 19 35.2l4.4 6.3c1.4 2 1.1 4.7-.6 6.4c-3.8 3.8-9.9 3.8-13.7 0L208.8 427.1c-3-3-7-4.7-11.3-4.7c-8.8 0-16 7.2-16 16v10.2c0 8.8 7.2 16 16 16h.5c8.8 0 16-7.2 16-16v-6.2c34 2.4 71 13 100 25.6c11 4.8 22.1 9.3 33 13.6l1 .4c12 4.6 27.3 9.3 41.4 9.3z"/></svg>', pt = ["😞", "😕", "😐", "🙂", "😄"], Kt = {
  star: we,
  heart: ba
}, Wt = {
  star: "#f59e0b",
...
function gr({
  ...
  if (t === "smiley") return _a({ max: s });
  if (t === "thumb") return va();
  if (t === "nps") return Ca();
  const r = Kt[t] || Kt.heart, i = n || Wt[t] || Wt.heart, o = e > 0 ? 1 / e : 2;
  return ({ value: a }) => {
    let l = parseFloat(a);
...
function _a({ max: t = 5 } = {}) {
  return ({ value: s }) => {
    ...
    const n = Math.min(
      pt.length - 1,
      Math.floor((e - 1) / (t - 1 || 1) * (pt.length - 1))
    );
    return u("span", {
      class: "sg-renderer-rating-smiley",
      title: `${e}/${t}`
    }, document.createTextNode(pt[n]));
  };
}
function va() {
  return ({ value: t }) => {
    ...
    return s > 0 ? (e.classList.add("is-up"), e.title = "Thumbs up", e.innerHTML = ya) : s < 0 ? (e.classList.add("is-down"), e.title = "Thumbs down", e.innerHTML = wa) : (e.classList.add("is-neutral"), e.title = "Neutral", e.append(document.createTextNode("—"))), e;
  };
}
function Ca() {
  return ({ value: t }) => {
...
function hr({
  ...
  return ({ value: c }) => {
    ...
    const g = s - t || 1, h = (S) => Math.max(t, Math.min(s, S)), y = (S) => (h(S) - t) / g * a, m = f && f.length ? f.map(Number) : [t + g * 0.6, t + g * 0.8], v = [t, ...m, s];
    let b = "";
    for (let S = 0; S < v.length - 1; S++) {
      const A = y(v[S]), $ = y(v[S + 1]) - A, L = r[S] || r[r.length - 1];
      b += `<rect x="${A.toFixed(2)}" y="0" width="${$.toFixed(2)}" height="${l}" fill="${L}"/>`;
    }
    const w = l * 0.42, C = (l - w) / 2;
    if (b += `<rect x="0" y="${C.toFixed(2)}" width="${y(d).toFixed(2)}" height="${w.toFixed(2)}" fill="${i}"/>`, p != null && Number.isFinite(p)) {
      const S = y(p), A = l * 0.85, $ = (l - A) / 2;
      b += `<rect x="${(S - 1).toFixed(2)}" y="${$.toFixed(2)}" width="2" height="${A.toFixed(2)}" fill="${o}"/>`;
    }
...
function br({
  ...
  return ({ value: l, td: c }) => {
    ...
    const g = Math.max(...f, 1), h = f.reduce((L, E) => L + E, 0), y = p && p.length ? 10 : 0, m = 1, v = 1, b = t - m * 2, w = s - v * 2 - y, C = Math.max(1, (b - (f.length - 1) * r) / f.length);
    let S = "";
    for (let L = 0; L < f.length; L++) {
      const E = f[L], N = E / g * w, D = m + L * (C + r), I = v + w - N, B = n ? E === g ? 1 : 0.45 : 0.85, H = p && p[L] != null ? `${p[L]}: ${E}` : `Bin ${L + 1}: ${E}`;
      S += `<rect x="${D.toFixed(2)}" y="${I.toFixed(2)}" width="${C.toFixed(2)}" height="${N.toFixed(2)}" fill="${a}" fill-opacity="${B}"><title>${xe(H)}</title></rect>`;
    }
    let A = "";
    if (p && p.length)
      for (let L = 0; L < f.length && L < p.length; L++) {
        const E = m + L * (C + r) + C / 2;
        A += `<text x="${E.toFixed(2)}" y="${(s - 1).toFixed(2)}" text-anchor="middle" font-size="7" fill="currentColor" opacity="0.65">${xe(p[L])}</text>`;
      }
    const $ = `<svg class="sg-renderer-histogram" viewBox="0 0 ${t} ${s}" width="${t}" height="${s}" preserveAspectRatio="none" aria-hidden="true">` + S + A + "</svg>";
    return o ? `<span class="sg-renderer-histogram-wrap">${$}<span class="sg-renderer-histogram-total">n=${h}</span></span>` : $;
  };
}
const _t = {
  red: "red",
  ...
  promoter: "green",
  safe: "green"
}, Sa = { red: "#ef4444", amber: "#f59e0b", green: "#10b981" };
function yr({
  size: t = 10,
  thresholds: s = null,
  inverted: e = !1,
  ...
  return ({ value: r }) => {
    ...
    } else if (i = _t[String(r).toLowerCase()] || null, !i) return "";
    const o = u("span", {
    ...
    return o.append(u("span", {
      ...
      style: `width:${t}px; height:${t}px; background:${Sa[i]};`,
      "aria-label": i
    })), n && o.append(u(
      "span",
...
function _r({
  ...
  return ({ value: e }) => {
    if (k(e)) return "";
    const n = String(e), r = u("span", { class: "sg-renderer-mentions" }), i = n.split(La);
    for (const o of i)
      if (o)
        if (o[0] === "@") {
          const a = o.slice(1), l = typeof t == "function" ? t(a) : null;
          r.append(qt(o, l, "sg-renderer-mention"));
        } else if (o[0] === "#") {
          const a = o.slice(1), l = typeof s == "function" ? s(a) : null;
          r.append(qt(o, l, "sg-renderer-hashtag"));
        } else
...
const ka = /^((25[0-5]|2[0-4]\d|[01]?\d?\d)\.){3}(25[0-5]|2[0-4]\d|[01]?\d?\d)$/, Ea = /^(([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|:(:[0-9a-fA-F]{1,4}){1,7}|::)$/;
function $a(t) {
  return ka.test(t);
}
function Ta(t) {
...
function xr({
  ...
  return ({ value: s, row: e }) => {
    if (k(s)) return "";
    const n = String(s).trim(), r = $a(n), i = !r && Ta(n);
    if (!r && !i)
      return u("span", {
...
function Tr({ width: t = 200, preload: s = "metadata" } = {}) {
...
function Rr({
  ...
  return ({ value: e }) => e == null || typeof e == "string" && (e === "" || s && Ra.has(e.trim().toLowerCase())) ? u(
    "span",
...
function Pr({ mask: t = !0 } = {}) {
  return ({ value: s, td: e }) => {
    if (e && e.classList.add("sg-renderer-number"), k(s)) return "";
    const n = String(s).replace(/\D/g, ""), r = n.length >= 13 && n.length <= 19, i = r && Pa(n), o = r ? Ia(n) : null, a = u("span", { class: `sg-renderer-card${i ? "" : " is-invalid"}` });
    o && a.append(u("span", {
...
function ue(t, s) {
  ...
  return t.color ? s.test(t.color) ? e.classList.add(`sg-pill-${t.color}`) : (e.style.background = t.color, e.style.color = lr(t.color)) : e.classList.add("sg-renderer-select-pill-bare"), t.icon && e.append(u("span", { class: "sg-renderer-select-pill-icon", "aria-hidden": "true" }, t.icon)), e.append(u(
    "span",
    { class: "sg-renderer-select-pill-label" },
    document.createTextNode(t.label)
  )), e;
}
const pe = /^(gray|red|orange|yellow|green|blue|indigo|purple|pink)$/;
function Et(t, s) {
...
function Vr({
  ...
  return (o) => {
    const { value: a, td: l } = o, c = Et(o, { options: i, placeholder: s, clearable: n, colorMap: r, editable: e });
    let d = i;
    if (i.length === 0 && c.options.length && (d = de(c.options), c.colorMap && typeof c.colorMap == "object"))
      for (const f of d)
    ...
    l && (l.classList.add("sg-renderer-select-cell"), l._sgSelectOpts = d, l._sgSelectClearable = c.clearable), c.editable && l && !l._sgSelectEditBound && (l._sgSelectEditBound = !0, l.addEventListener("dblclick", (f) => {
      f._sgSelectHandled || (f._sgSelectHandled = !0, f.stopPropagation(), Fr(l, o));
    }));
    const p = d.find((f) => String(f.value) === String(a)) || null;
    return p ? ue(p, pe) : k(a) ? u(
      "span",
...
function Fr(t, s) {
  ...
  for (const p of e) {
    ...
    f.append(ue(p, pe)), f.addEventListener("click", () => l(p.value)), a.append(f);
  }
  function c(p) {
    p.key === "Escape" && (p.stopPropagation(), $e());
  }
  function d(p) {
    !a.contains(p.target) && !t.contains(p.target) && $e();
  }
  document.addEventListener("keydown", c), setTimeout(() => document.addEventListener("mousedown", d), 0), document.body.appendChild(a), U(a, t), Oe = { pop: a, onKey: c, onDocClick: d, anchor: t };
}
function $e() {
  if (!Oe) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = Oe;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), Oe = null, q(n);
}
function Br(t) {
...
function Hr({
  ...
  if (r && typeof r == "object")
    for (const o of i)
      !o.color && Object.prototype.hasOwnProperty.call(r, o.value) && (o.color = r[o.value]);
  return (o) => {
    const { value: a, td: l } = o, c = Et(o, { options: i, placeholder: e, colorMap: r, editable: n, separator: s });
    let d = i;
    if (i.length === 0 && c.options.length && (d = de(c.options), c.colorMap && typeof c.colorMap == "object"))
      for (const g of d)
    ...
    l && (l.classList.add("sg-renderer-multiselect-cell"), l._sgMultiOpts = d, l._sgMultiSep = c.separator), c.editable && l && !l._sgMultiEditBound && (l._sgMultiEditBound = !0, l.addEventListener("dblclick", (g) => {
      g._sgMultiHandled || (g._sgMultiHandled = !0, g.stopPropagation(), Va(l, o));
    }));
    const p = Br(a);
    if (!p.length)
      return u(
...
function Va(t, s) {
  ft();
  const e = t._sgMultiOpts || [], n = t._sgMultiSep || ",", { row: r, col: i } = s, o = Br(r && i?.field != null ? r[i.field] : null), a = new Set(o), l = u("div", { class: "sg-renderer-multiselect-popover", role: "listbox", "aria-multiselectable": "true" });
  l.addEventListener("mousedown", (h) => h.stopPropagation());
  function c(h) {
    const y = a.has(String(h.value)), m = u("button", {
      type: "button",
    ...
    )), m.append(ue(h, pe)), m.addEventListener("click", () => {
  ...
  d();
  function p() {
    const { api: h } = s, y = Array.from(a), m = r && i?.field != null ? r[i.field] : null, v = Array.isArray(m) || m == null ? y : y.join(n), b = m;
    ...
    w && w.dispatchEvent(new CustomEvent("grid:cellValueChanged", {
      bubbles: !0,
      detail: { rowId: r?.id ?? r?._sg_id, colId: i?.field, oldValue: b, newValue: v }
    })), ft();
  }
  function f(h) {
    h.key === "Escape" && (h.stopPropagation(), ft()), h.key === "Enter" && (h.stopPropagation(), h.preventDefault(), p());
  }
  function g(h) {
    !l.contains(h.target) && !t.contains(h.target) && p();
  }
  document.addEventListener("keydown", f), setTimeout(() => document.addEventListener("mousedown", g), 0), document.body.appendChild(l), U(l, t), Ge = { pop: l, onKey: f, onDocClick: g, anchor: t };
}
function ft() {
  if (!Ge) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = Ge;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), Ge = null, q(n);
}
function Or({
  options: t = [],
  ...
  if (r && typeof r == "object")
    for (const o of i)
      !o.color && Object.prototype.hasOwnProperty.call(r, o.value) && (o.color = r[o.value]);
  return (o) => {
    const { value: a, td: l } = o, c = Et(o, { options: i, placeholder: s, colorMap: r, editable: e }), d = o?.col?.cellRendererConfig?.allowCustom ?? n;
    let p = i;
    if (i.length === 0 && c.options.length && (p = de(c.options), c.colorMap && typeof c.colorMap == "object"))
      for (const g of p)
        !g.color && Object.prototype.hasOwnProperty.call(c.colorMap, g.value) && (g.color = c.colorMap[g.value]);
    l && (l.classList.add("sg-renderer-combobox-cell"), l._sgComboOpts = p, l._sgComboAllowCustom = d, l._sgComboPlaceholder = c.placeholder), c.editable && l && !l._sgComboEditBound && (l._sgComboEditBound = !0, l.addEventListener("dblclick", (g) => {
      g._sgComboHandled || (g._sgComboHandled = !0, g.stopPropagation(), Fa(l, o));
    }));
    const f = p.find((g) => String(g.value) === String(a)) || null;
    return f ? ue(f, pe) : k(a) ? u(
      "span",
      { class: "sg-renderer-select-placeholder" },
      document.createTextNode(c.placeholder)
    ) : u("span", { class: "sg-renderer-select-bare" }, document.createTextNode(String(a)));
...
function Fa(t, s) {
  ...
  function f(b) {
    ...
    i && o?.field != null && (i[o.field] = b), w?.applyTransaction && w.applyTransaction({ update: [i] });
    const S = t.closest('[data-controller~="grid"]');
  ...
  function g() {
    ...
    return b ? e.filter((w) => String(w.label).toLowerCase().includes(b)) : e;
  }
  function h() {
    p.replaceChildren();
    const b = g();
    if (l >= b.length && (l = Math.max(0, b.length - 1)), b.forEach((w, C) => {
      const S = u("button", {
        type: "button",
      ...
      S.append(ue(w, pe)), S.addEventListener("mouseenter", () => {
        l = C, y();
  ...
  function y() {
    p.querySelectorAll(".sg-renderer-combobox-option").forEach((b, w) => {
      b.classList.toggle("is-highlighted", w === l), b.setAttribute("aria-selected", w === l ? "true" : "false");
    });
  }
  d.addEventListener("input", () => {
    ...
    const w = g();
    b.key === "ArrowDown" ? (b.preventDefault(), l = Math.min(w.length - 1, l + 1), y()) : b.key === "ArrowUp" ? (b.preventDefault(), l = Math.max(0, l - 1), y()) : b.key === "Enter" ? (b.preventDefault(), w[l] ? f(w[l].value) : n && a.trim() && f(a.trim())) : b.key === "Escape" && (b.stopPropagation(), ge());
  });
  function m(b) {
    b.key === "Escape" && (b.stopPropagation(), ge());
  }
  function v(b) {
    !c.contains(b.target) && !t.contains(b.target) && ge();
  }
  document.addEventListener("keydown", m), setTimeout(() => document.addEventListener("mousedown", v), 0), document.body.appendChild(c), U(c, t), h(), setTimeout(() => d.focus(), 0), ze = { pop: c, onKey: m, onDocClick: v, anchor: t };
}
function ge() {
  if (!ze) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = ze;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), ze = null, q(n);
}
function Ce(t) {
  if (!t) return "";
...
function Gr({
  ...
  return (l) => {
    const { value: c, td: d } = l, p = l?.col?.cellRendererConfig || {}, f = p.min ? j(p.min) : r ? j(r) : null, g = p.max ? j(p.max) : i ? j(i) : null, h = p.firstDayOfWeek ?? o, y = p.editable ?? e;
    d && (d.classList.add("sg-renderer-datepicker-cell"), d._sgDatePickerMin = f, d._sgDatePickerMax = g, d._sgDatePickerFdow = h), y && d && !d._sgDatePickerBound && (d._sgDatePickerBound = !0, d.addEventListener("dblclick", (v) => {
      v._sgDatePickerHandled || (v._sgDatePickerHandled = !0, v.stopPropagation(), Ha(d, l));
    }));
    const m = j(c);
    return m ? u(
      "span",
...
const zr = [
  ...
  "November",
  "December"
], jr = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
function Ba(t, s, e, n, r, i, o) {
  ...
  for (let b = 0; b < 7; b++)
    f.append(u(
      ...
      document.createTextNode(jr[(b + o) % 7])
    ));
  const g = u("div", { class: "sg-renderer-datepicker-grid" }), y = (new Date(t, s, 1).getDay() - o + 7) % 7, m = new Date(t, s, 1 - y), v = /* @__PURE__ */ new Date();
  for (let b = 0; b < 42; b++) {
    const w = new Date(m.getFullYear(), m.getMonth(), m.getDate() + b), C = w.getMonth() === s, S = ce(w, e), A = ce(w, v), $ = r && w < r || i && w > i, L = ["sg-renderer-datepicker-day"];
    C || L.push("is-other-month"), S && L.push("is-selected"), A && L.push("is-today"), $ && L.push("is-disabled");
    const E = u("button", {
      type: "button",
      class: L.join(" "),
      disabled: $ ? "" : null,
    ...
    E.addEventListener("click", () => n(w)), g.append(E);
  }
...
function Ha(t, s) {
  ...
  function f(m) {
    const { api: v } = s, b = e && n?.field != null ? e[n.field] : null, w = m ? Ce(m) : null;
    e && n?.field != null && (e[n.field] = w), v?.applyTransaction && v.applyTransaction({ update: [e] });
    const C = t.closest('[data-controller~="grid"]');
    C && C.dispatchEvent(new CustomEvent("grid:cellValueChanged", {
      bubbles: !0,
      detail: { rowId: e?.id ?? e?._sg_id, colId: n?.field, oldValue: b, newValue: w }
    })), Te();
  }
  function g() {
    p.replaceChildren();
    const { wrap: m, prev: v, next: b } = Ba(i, o, a, f, l, c, d);
  ...
  function h(m) {
    m.key === "Escape" && (m.stopPropagation(), Te());
  }
  function y(m) {
    !p.contains(m.target) && !t.contains(m.target) && Te();
  }
  document.addEventListener("keydown", h), setTimeout(() => document.addEventListener("mousedown", y), 0), document.body.appendChild(p), g(), U(p, t), je = { pop: p, onKey: h, onDocClick: y, anchor: t };
}
function Te() {
  if (!je) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = je;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), je = null, q(n);
}
function Ur({
  style: t = "24h",
  // '24h' | '12h'
  ...
  return (r) => {
    ...
    o && (o.classList.add("sg-renderer-timepicker-cell"), o._sgTimePickerStyle = l, o._sgTimePickerStep = c), d && o && !o._sgTimePickerBound && (o._sgTimePickerBound = !0, o.addEventListener("dblclick", (f) => {
      f._sgTimePickerHandled || (f._sgTimePickerHandled = !0, f.stopPropagation(), Ga(o, r));
    }));
    const p = kt(i);
    return p ? u(
      "span",
      { class: "sg-renderer-timepicker-value" },
      document.createTextNode(Oa(p, l))
    ) : n;
  };
}
function Oa(t, s) {
...
function Ga(t, s) {
  he();
  const e = t._sgTimePickerStyle || "24h", n = t._sgTimePickerStep || 5, { row: r, col: i } = s, o = kt(r && i?.field != null ? r[i.field] : null) || { h: 9, m: 0 };
  ...
  function d(E) {
    const { api: N } = s, D = r && i?.field != null ? r[i.field] : null;
    r && i?.field != null && (r[i.field] = E), N?.applyTransaction && N.applyTransaction({ update: [r] });
  ...
  function p() {
    const E = String(a).padStart(2, "0"), N = String(l).padStart(2, "0");
    d(`${E}:${N}`);
  }
  ...
  function h() {
    g.replaceChildren();
    const E = e === "12h" ? Array.from({ length: 12 }, (N, D) => D === 0 ? 12 : D) : Array.from({ length: 24 }, (N, D) => D);
    for (const N of E) {
      const D = e === "12h" ? a >= 12 ? N === 12 ? 12 : N + 12 : N === 12 ? 0 : N : N, I = D === a, B = u("button", {
        type: "button",
        class: `sg-renderer-timepicker-item${I ? " is-selected" : ""}`
      }, document.createTextNode(e === "12h" ? String(N) : String(N).padStart(2, "0")));
      B.addEventListener("click", () => {
        a = D, h();
      }), B.addEventListener("dblclick", () => {
        a = D, p();
      }), g.append(B), I && setTimeout(() => B.scrollIntoView({ block: "nearest" }), 0);
    }
  }
  ...
  function v() {
    ...
    for (let E = 0; E < 60; E += n) {
      const N = E === l, D = u("button", {
        type: "button",
        class: `sg-renderer-timepicker-item${N ? " is-selected" : ""}`
      }, document.createTextNode(String(E).padStart(2, "0")));
      D.addEventListener("click", () => {
        l = E, v();
      }), D.addEventListener("dblclick", () => {
        l = E, p();
      }), m.append(D), N && setTimeout(() => D.scrollIntoView({ block: "nearest" }), 0);
    }
  }
  const b = u("div", { class: "sg-renderer-timepicker-cols" });
  if (b.append(f, y), e === "12h") {
    ...
    const N = u("div", { class: "sg-renderer-timepicker-list" });
    for (const D of ["AM", "PM"]) {
      const I = D === "AM" && a < 12 || D === "PM" && a >= 12, B = u("button", {
        type: "button",
        class: `sg-renderer-timepicker-item${I ? " is-selected" : ""}`
      }, document.createTextNode(D));
      B.addEventListener("click", () => {
        D === "AM" && a >= 12 && (a -= 12), D === "PM" && a < 12 && (a += 12), h(), N.querySelectorAll(".sg-renderer-timepicker-item").forEach((H, F) => {
          H.classList.toggle("is-selected", F === 0 && a < 12 || F === 1 && a >= 12);
        });
      }), N.append(B);
    }
    E.append(N), b.append(E);
  }
  ...
  C.addEventListener("click", () => he()), A.addEventListener("click", () => d(null)), S.addEventListener("click", () => p()), w.append(A, C, S), c.append(b, w);
  function $(E) {
    E.key === "Escape" && (E.stopPropagation(), he()), E.key === "Enter" && (E.stopPropagation(), E.preventDefault(), p());
  }
  function L(E) {
    !c.contains(E.target) && !t.contains(E.target) && he();
  }
  document.addEventListener("keydown", $), setTimeout(() => document.addEventListener("mousedown", L), 0), document.body.appendChild(c), h(), v(), U(c, t), Ue = { pop: c, onKey: $, onDocClick: L, anchor: t };
}
function he() {
  if (!Ue) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = Ue;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), Ue = null, q(n);
}
function Kr(t) {
  if (t == null || t === "") return null;
  let s, e;
...
function Wr({
  ...
  return (r) => {
    ...
    o && (o.classList.add("sg-renderer-daterange-cell"), o._sgRangeFdow = l), c && o && !o._sgRangeBound && (o._sgRangeBound = !0, o.addEventListener("dblclick", (p) => {
      p._sgRangeHandled || (p._sgRangeHandled = !0, p.stopPropagation(), ja(o, r));
    }));
    const d = Kr(i);
    return d ? u(
      "span",
      { class: "sg-renderer-daterange-value" },
      document.createTextNode(za(d, t))
    ) : e;
  };
}
let Ke = null;
function ja(t, s) {
  Ae();
  const { row: e, col: n } = s, r = Kr(e && n?.field != null ? e[n.field] : null) || { start: null, end: null };
  ...
  function p() {
    const { api: v } = s, b = e && n?.field != null ? e[n.field] : null, w = i || o ? { start: i ? Ce(i) : null, end: o ? Ce(o) : null } : null;
    e && n?.field != null && (e[n.field] = w), v?.applyTransaction && v.applyTransaction({ update: [e] });
    const C = t.closest('[data-controller~="grid"]');
    C && C.dispatchEvent(new CustomEvent("grid:cellValueChanged", {
      bubbles: !0,
  ...
  function g(v, b) {
    const w = u("div", { class: "sg-renderer-datepicker-cal" }), C = u("div", { class: "sg-renderer-datepicker-head" }), S = u(
      ...
      "span",
      { class: "sg-renderer-datepicker-title" },
      document.createTextNode(`${zr[b]} ${v}`)
    ), $ = u(
      "button",
      { type: "button", class: "sg-renderer-datepicker-nav" },
      document.createTextNode("›")
    ...
    for (let H = 0; H < 7; H++)
      L.append(u(
        "span",
        { class: "sg-renderer-datepicker-dow" },
        document.createTextNode(jr[(H + c) % 7])
      ));
    const E = u("div", { class: "sg-renderer-datepicker-grid" }), D = (new Date(v, b, 1).getDay() - c + 7) % 7, I = new Date(v, b, 1 - D), B = /* @__PURE__ */ new Date();
    for (let H = 0; H < 42; H++) {
      const F = new Date(I.getFullYear(), I.getMonth(), I.getDate() + H), X = F.getMonth() === b, ee = ce(F, i), Z = ce(F, o), te = i && o && F > i && F < o, O = ce(F, B), R = ["sg-renderer-datepicker-day"];
      X || R.push("is-other-month"), (ee || Z) && R.push("is-selected"), te && R.push("is-in-range"), O && R.push("is-today");
      const G = u(
        "button",
        { type: "button", class: R.join(" "), title: Ce(F) },
        document.createTextNode(String(F.getDate()))
      );
      G.addEventListener("click", () => f(F)), E.append(G);
    }
    return w.append(C, L, E), { wrap: w, prev: S, next: $ };
  }
  function h() {
    d.replaceChildren();
    const v = u("div", { class: "sg-renderer-daterange-months" }), b = l === 11 ? a + 1 : a, w = (l + 1) % 12, C = g(a, l), S = g(b, w);
    C.prev.addEventListener("click", () => {
      l === 0 ? (l = 11, a -= 1) : l -= 1, h();
    }), S.next.addEventListener("click", () => {
      l === 11 ? (l = 0, a += 1) : l += 1, h();
    }), C.next.style.visibility = "hidden", S.prev.style.visibility = "hidden", v.append(C.wrap, S.wrap);
    const A = u("div", { class: "sg-renderer-datepicker-footer" }), $ = u(
      "button",
      { type: "button", class: "sg-renderer-datepicker-clear" },
      document.createTextNode("Clear")
    ), L = u(
    ...
    $.addEventListener("click", () => {
      i = null, o = null, p();
    }), L.addEventListener("click", p), A.append($, L), d.append(v, A);
  }
  function y(v) {
    v.key === "Escape" && (v.stopPropagation(), Ae()), v.key === "Enter" && (v.stopPropagation(), v.preventDefault(), p());
  }
  function m(v) {
    !d.contains(v.target) && !t.contains(v.target) && Ae();
  }
  document.addEventListener("keydown", y), setTimeout(() => document.addEventListener("mousedown", m), 0), document.body.appendChild(d), h(), U(d, t), Ke = { pop: d, onKey: y, onDocClick: m, anchor: t };
}
function Ae() {
  if (!Ke) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = Ke;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), Ke = null, q(n);
}
const qr = [
  "#ef4444",
  "#f97316",
...
function Yr({
  ...
  return (o) => {
    ...
    if (l && (l.classList.add("sg-renderer-colorpicker-cell"), l._sgPickerPalette = d), h && l && !l._sgPickerBound && (l._sgPickerBound = !0, l.addEventListener("dblclick", (v) => {
      v._sgPickerHandled || (v._sgPickerHandled = !0, v.stopPropagation(), Ua(l, o));
    })), k(a)) return i;
    const y = u("span", { class: "sg-renderer-swatch" }), m = String(a).toLowerCase() === "#ffffff" ? " border: 1px solid #d1d5db;" : "";
...
function Ua(t, s) {
  Ne();
  const e = t._sgPickerPalette || qr, { row: n, col: r } = s, i = n && r?.field != null ? n[r.field] : null, o = u("div", { class: "sg-renderer-colorpicker-popover", role: "dialog" });
  o.addEventListener("mousedown", (m) => m.stopPropagation());
  function a(m) {
    const { api: v } = s, b = n && r?.field != null ? n[r.field] : null;
  ...
  for (const m of e) {
    const v = String(i).toLowerCase() === String(m).toLowerCase(), b = u("button", {
      type: "button",
  ...
  function h(m) {
    if (m.key === "Escape" && (m.stopPropagation(), Ne()), m.key === "Enter") {
      m.stopPropagation();
  ...
  function y(m) {
    !o.contains(m.target) && !t.contains(m.target) && Ne();
  }
  document.addEventListener("keydown", h), setTimeout(() => document.addEventListener("mousedown", y), 0), document.body.appendChild(o), U(o, t), We = { pop: o, onKey: h, onDocClick: y, anchor: t };
}
function Ne() {
  if (!We) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = We;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), We = null, q(n);
}
function Zr({
  lines: t = 3,
  rows: s = 6,
...
`,
  ...
  return (o) => {
    ...
    if (l && (l.classList.add("sg-renderer-multiline"), l._sgTextareaRows = p, l._sgTextareaCols = f, l._sgTextareaSep = g), h && l && !l._sgTextareaBound && (l._sgTextareaBound = !0, l.addEventListener("dblclick", (m) => {
      m._sgTextareaHandled || (m._sgTextareaHandled = !0, m.stopPropagation(), $t(l, o));
    })), k(a)) return i;
    const y = String(a);
    if (d != null && d > 0) {
      const m = u("div", {
...
function $t(t, s) {
  ...
  function c() {
    const { api: m } = s, v = l.value, b = r && i?.field != null ? r[i.field] : null;
  ...
  }, 0), qe = { pop: a, onKey: h, onDocClick: y, anchor: t };
}
function oe() {
  if (!qe) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = qe;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), qe = null, q(n);
}
function at(t, s, e, n) {
...
function Xr({
  ...
  return (o) => {
    ...
    return p && y.append(u("span", { class: "sg-renderer-action-icon", "aria-hidden": "true" }, p)), y.append(u("span", { class: "sg-renderer-action-label" }, document.createTextNode(d))), y.addEventListener("click", (m) => {
      m.stopPropagation(), !h && (typeof r == "function" && r(l, o), g && at(a, o, g));
    }), y;
  };
}
const Ka = '<svg viewBox="0 0 16 16" aria-hidden="true"><circle cx="8" cy="3" r="1.5" fill="currentColor"/><circle cx="8" cy="8" r="1.5" fill="currentColor"/><circle cx="8" cy="13" r="1.5" fill="currentColor"/></svg>';
function Jr({
  items: t = [],
  icon: s = Ka,
...
function Qr(t, s, e) {
  ...
  for (const o of e) {
    ...
    a.icon && c.append(u("span", { class: "sg-renderer-menu-icon", "aria-hidden": "true" }, a.icon)), c.append(u("span", { class: "sg-renderer-menu-label" }, document.createTextNode(a.label))), a.shortcut && c.append(u("span", { class: "sg-renderer-menu-shortcut" }, document.createTextNode(a.shortcut))), c.addEventListener("click", () => {
      a.disabled || (Me(), typeof a.onClick == "function" && a.onClick(s.row, s), a.action && at(t, s, a.action));
    }), n.append(c);
  }
  function r(o) {
    o.key === "Escape" && (o.stopPropagation(), Me());
  }
  function i(o) {
    !n.contains(o.target) && !t.contains(o.target) && Me();
  }
  document.addEventListener("keydown", r), setTimeout(() => document.addEventListener("mousedown", i), 0), document.body.appendChild(n), U(n, t), Ye = { pop: n, onKey: r, onDocClick: i, anchor: t };
}
function Me() {
  if (!Ye) return;
  const { pop: t, onKey: s, onDocClick: e, anchor: n } = Ye;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), Ye = null, q(n);
}
function es({
  ...
  return (n) => {
    ...
    o.icon && d.append(u("span", { class: "sg-renderer-action-icon", "aria-hidden": "true" }, o.icon)), d.append(u("span", { class: "sg-renderer-action-label" }, document.createTextNode(o.label))), d.addEventListener("click", (f) => {
      f.stopPropagation(), typeof o.onClick == "function" && o.onClick(n.row, n), o.action && at(r, n, o.action);
    });
    const p = u(
      "button",
  ...
  return (s) => {
    ...
    for (const o of r) {
      ...
      a.addEventListener("click", (l) => {
        l.stopPropagation(), typeof o.onClick == "function" && o.onClick(s.row, s), o.name && at(e, s, o.name);
      }), i.append(a);
    }
    return i;
  };
}
const qa = '<svg viewBox="0 0 16 16" aria-hidden="true"><circle cx="6" cy="3" r="1.2" fill="currentColor"/><circle cx="10" cy="3" r="1.2" fill="currentColor"/><circle cx="6" cy="8" r="1.2" fill="currentColor"/><circle cx="10" cy="8" r="1.2" fill="currentColor"/><circle cx="6" cy="13" r="1.2" fill="currentColor"/><circle cx="10" cy="13" r="1.2" fill="currentColor"/></svg>';
function ns({ label: t = "Drag to reorder" } = {}) {
  return (s) => {
    const { td: e } = s;
...
function rs({ startAt: t = 1, padTo: s = 0 } = {}) {
  return (e) => {
...
function ss() {
  return (t) => {
    ...
    return r.innerHTML = un, r.addEventListener("mousedown", (i) => i.stopPropagation()), r.addEventListener("click", (i) => {
      i.stopPropagation();
...
const Ya = /^[0-9a-f]{8}-?[0-9a-f]{4}-?[0-9a-f]{4}-?[0-9a-f]{4}-?[0-9a-f]{12}$/i;
function Za(t) {
  const s = String(t).toLowerCase();
  return s.length <= 13 ? s : `${s.slice(0, 8)}…${s.slice(-4)}`;
}
function is({ short: t = !0, copy: s = !0 } = {}) {
  return ({ value: e, td: n }) => {
    if (k(e)) return "";
    n && n.classList.add("sg-renderer-uuid-cell");
    const r = String(e), i = Ya.test(r), o = t ? Za(r) : r, a = u("span", {
...
const Xa = /^[0-9a-f]{4,64}$/i;
function os({ length: t = 7, href: s = null, copy: e = !0 } = {}) {
  return ({ value: n, td: r }) => {
    if (k(n)) return "";
    r && r.classList.add("sg-renderer-gitsha-cell"), r?._sgPickerPalette;
    const i = String(n).trim(), o = Xa.test(i), a = o ? i.slice(0, t) : i, l = u("span", {
...
const Ja = /^(?:[0-9a-f]{2}[:-]?){5}[0-9a-f]{2}$|^(?:[0-9a-f]{4}\.){2}[0-9a-f]{4}$/i;
function as({ vendorLookup: t = null } = {}) {
  return ({ value: s, td: e }) => {
    if (k(s)) return "";
    e && e.classList.add("sg-renderer-mac-cell");
    const n = String(s).trim(), r = Ja.test(n), i = n.replace(/[^0-9a-f]/gi, "").toLowerCase(), o = i.length === 12 ? `${i.slice(0, 2)}:${i.slice(2, 4)}:${i.slice(4, 6)}:${i.slice(6, 8)}:${i.slice(8, 10)}:${i.slice(10, 12)}` : n, a = i.slice(0, 6), l = typeof t == "function" ? t(a) : null;
    return u("span", {
...
function ls({ groups: t = 4, groupLen: s = 4, mask: e = !1 } = {}) {
  return ({ value: n, td: r }) => {
    if (k(n)) return "";
...
const Qa = /^[A-HJ-NPR-Z0-9]{17}$/;
function cs({} = {}) {
  return ({ value: t, td: s }) => {
    if (k(t)) return "";
    s && s.classList.add("sg-renderer-vin-cell");
    const e = String(t).trim().toUpperCase(), n = Qa.test(e), r = n ? `${e.slice(0, 3)} ${e.slice(3, 9)} ${e.slice(9)}` : e;
    return u(
      "span",
...
function ds({} = {}) {
  return ({ value: t, td: s }) => {
    ...
    let n, r;
    return e.length === 13 ? (n = el(e), r = /^\d{13}$/.test(e)) : e.length === 10 ? (n = tl(e), r = /^\d{9}[\dXx]$/.test(e)) : (n = String(t), r = !1), u(
      "span",
...
const nl = /* @__PURE__ */ new Set(["B", "I", "EM", "STRONG", "U", "S", "DEL", "CODE", "A", "BR", "SPAN"]);
function rl(t) {
...
function us({ editable: t = !1, rows: s = 8, cols: e = 60 } = {}) {
  return (n) => {
    ...
    if (i && (i.classList.add("sg-renderer-html-cell"), t && !i._sgHtmlBound && (i._sgHtmlBound = !0, i._sgTextareaRows = s, i._sgTextareaCols = e, i.addEventListener("dblclick", (a) => {
      a._sgTextareaHandled || (a._sgTextareaHandled = !0, a.stopPropagation(), $t(i, n));
    }))), k(r)) return "";
    const o = u("span", { class: "sg-renderer-html" });
    return o.innerHTML = rl(String(r)), o;
  };
}
function ps({ maxLines: t = 4, editable: s = !1, rows: e = 12, cols: n = 60 } = {}) {
  return (r) => {
    ...
    if (o && (o.classList.add("sg-renderer-yaml-cell"), s && !o._sgYamlBound && (o._sgYamlBound = !0, o._sgTextareaRows = e, o._sgTextareaCols = n, o.addEventListener("dblclick", (c) => {
      c._sgTextareaHandled || (c._sgTextareaHandled = !0, c.stopPropagation(), $t(o, r));
    }))), k(i)) return "";
...
const sl = /\bhttps?:\/\/[^\s<>"']+/g, il = /\b[\w.+-]+@[\w-]+\.[\w.-]+\b/g;
function gs({} = {}) {
  return ({ value: t, td: s }) => {
    if (k(t)) return "";
    s && s.classList.add("sg-renderer-autolink-cell");
    let e = xe(String(t));
    e = e.replace(sl, (r) => `<a class="sg-renderer-link" href="${r}" target="_blank" rel="noopener noreferrer">${r}</a>`), e = e.replace(il, (r) => `<a class="sg-renderer-link" href="mailto:${r}">${r}</a>`);
...
function ol(t, s = 16) {
  ...
  if (i === 0) return `${e}${n}`;
  if (i === o) return `${e}${n + 1}`;
  const l = bs(i, o), c = i / l, d = o / l;
  return n === 0 ? `${e}${c}/${d}` : `${e}${n} ${c}/${d}`;
}
function ys({ maxDenom: t = 16 } = {}) {
  return ({ value: s, td: e }) => {
    if (e && e.classList.add("sg-renderer-number"), k(s)) return "";
    const n = Number(s);
...
const al = ["⁰", "¹", "²", "³", "⁴", "⁵", "⁶", "⁷", "⁸", "⁹"];
function ll(t) {
  return String(t).split("").map((s) => s === "-" ? "⁻" : al[Number(s)] || s).join("");
}
function ws({
  decimals: t = 2,
  pretty: s = !0
...
const cl = '<path fill="currentColor" d="M3 6v4h3l4 3V3L6 6H3z"/>', gt = '<path fill="none" stroke="currentColor" stroke-width="1.4" d="M12 6.5q1 1 0 3"/>', Yt = '<path fill="none" stroke="currentColor" stroke-width="1.4" d="M14 5q2 2 0 6"/>', dl = '<path fill="none" stroke="currentColor" stroke-width="1.4" d="M16 3.5q3 3 0 9"/>', ul = '<line x1="13" y1="4" x2="17" y2="9" stroke="currentColor" stroke-width="1.4"/><line x1="17" y1="4" x2="13" y2="9" stroke="currentColor" stroke-width="1.4"/>';
function xs({
  showValue: t = !1,
  editable: s = !1
} = {}) {
  return (e) => {
    ...
    if (r && (r.classList.add("sg-renderer-volume-cell"), s && !r._sgVolumeBound && (r._sgVolumeBound = !0, r.addEventListener("dblclick", (c) => {
      c._sgVolumeHandled || (c._sgVolumeHandled = !0, c.stopPropagation(), pl(r, e));
    }))), k(n)) return "";
    let i = Number(n);
    if (!Number.isFinite(i)) return String(n);
    i = Math.max(0, Math.min(100, i));
    let o = "";
    i === 0 ? o = ul : i < 33 ? o = gt : i < 66 ? o = gt + Yt : o = gt + Yt + dl;
    const a = u("span", { class: "sg-renderer-volume", title: `${Math.round(i)}%` }), l = u("span", { class: "sg-renderer-volume-icon", "aria-hidden": "true" });
    return l.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 16" width="20" height="14">${cl}${o}</svg>`, a.append(l), t && a.append(u(
      "span",
...
function pl(t, s) {
  ...
  i.append(o, a), o.addEventListener("keydown", (p) => {
    p.key === "Enter" ? (p.preventDefault(), l()) : p.key === "Escape" && (p.stopPropagation(), me());
  }), o.addEventListener("change", l);
  function c(p) {
    p.key === "Escape" && (p.stopPropagation(), me());
  }
  function d(p) {
    !i.contains(p.target) && !t.contains(p.target) && me();
  }
  document.addEventListener("keydown", c), setTimeout(() => document.addEventListener("mousedown", d), 0), document.body.appendChild(i), U(i, t), setTimeout(() => o.focus(), 0), Ze = { pop: i, onKey: c, onDocClick: d, anchor: t };
}
const fl = [
  { match: /^image\//, icon: "🖼️" },
  { match: /^audio\//, icon: "🎵" },
...
function Es({
  ...
  return ({ value: s, td: e }) => {
    ...
    const n = ks(s);
    if (!n) return "";
    const r = Ss(n.filename, n.content_type), i = u("a", {
    ...
    return i.append(u(
      ...
      document.createTextNode(Ls(n.byte_size))
    )), i;
  };
}
const gl = '<svg viewBox="0 0 16 16" aria-hidden="true"><path fill="currentColor" d="M8 1a1 1 0 011 1v6.586l1.293-1.293a1 1 0 011.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 011.414-1.414L7 8.586V2a1 1 0 011-1zm-6 11a1 1 0 011 1v1h10v-1a1 1 0 112 0v2a1 1 0 01-1 1H2a1 1 0 01-1-1v-2a1 1 0 011-1z"/></svg>';
function $s({
  label: t = "Download"
} = {}) {
  return ({ value: s, td: e }) => {
    ...
    i.innerHTML = gl, r.append(i);
    let o = t;
    return n.byte_size && (o += ` (${Ls(n.byte_size)})`), r.append(u("span", {}, document.createTextNode(o))), r;
  };
}
function Ts({ size: t = 18 } = {}) {
  return ({ value: s, td: e }) => {
    if (e && e.classList.add("sg-renderer-mime-icon-cell"), k(s)) return "";
    const n = typeof s == "object" ? s : { content_type: String(s), filename: String(s) }, r = Ss(n.filename, n.content_type);
    return u("span", {
...
function As({
  ...
  return (n) => {
    const { value: r, td: i } = n;
    if (i && i.classList.add("sg-renderer-gallery-cell"), k(r)) return "";
    ...
    i && !i._sgGalleryBound && (i._sgGalleryBound = !0, i.addEventListener("dblclick", (d) => {
      d._sgGalleryHandled || (d._sgGalleryHandled = !0, d.stopPropagation(), hl(i, o, e));
    }));
...
function ht() {
  ...
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), Xe = null, q(n);
}
function hl(t, s, e) {
  ht();
  ...
  function r(o) {
    o.key === "Escape" && (o.stopPropagation(), ht());
  }
  function i(o) {
    !n.contains(o.target) && !t.contains(o.target) && ht();
  }
  document.addEventListener("keydown", r), setTimeout(() => document.addEventListener("mousedown", i), 0), document.body.appendChild(n), U(n, t), Xe = { pop: n, onKey: r, onDocClick: i, anchor: t };
}
function ml(t) {
  let s = 0;
...
function Ns({
  ...
  return ({ value: i, td: o }) => {
    if (o && o.classList.add("sg-renderer-waveform-cell"), k(i)) return "";
    let a;
    if (Array.isArray(i))
      a = i.map(Number);
    else {
    ...
    let p = "";
    for (let h = 0; h < l; h++) {
      const m = Math.max(0.05, Math.min(1, a[h])) * s, v = h * c + d / 2, b = (s - m) / 2;
      p += `<rect x="${v.toFixed(2)}" y="${b.toFixed(2)}" width="${(c - d).toFixed(2)}" height="${m.toFixed(2)}" rx="0.6" fill="${n}"/>`;
    }
...
function Ps({
  ...
  return ({ value: s, row: e, td: n }) => {
    ...
    let o = i ? Xt[i] : null;
    if (!o) {
      for (const l of Object.values(Xt))
        if (l.re.test(r)) {
          o = l;
          break;
        }
...
const yl = '<svg viewBox="0 0 16 16" aria-hidden="true"><path fill="currentColor" d="M8 1.5a6.5 6.5 0 110 13 6.5 6.5 0 010-13zm0 3a.75.75 0 00-.75.75v3.5a.75.75 0 001.5 0V5.25A.75.75 0 008 4.5zm0 6.5a1 1 0 100 2 1 1 0 000-2z"/></svg>';
function Fs({
  icon: t = yl,
  retryLabel: s = "Retry"
} = {}) {
...
function Bs({} = {}) {
  return ({ value: t, td: s }) => {
    ...
    const e = String(t).toLowerCase(), n = wl[e] || { color: "gray", icon: "·", label: String(t) }, r = u("span", { class: `sg-pill sg-pill-${n.color}`, title: n.label });
...
function Gs({
  ...
  return ({ value: e, td: n }) => {
    ...
    const i = u("span", { class: "sg-renderer-countdown", title: r.toLocaleString() }), o = () => {
      ...
      i.textContent = l <= 0 ? s : _l(l), i.classList.toggle("is-expired", l <= 0);
    };
    o();
    const a = setInterval(() => {
...
function js({
  ...
  return ({ value: n, td: r }) => {
    ...
    switch (t) {
      case "week":
        o = `W${String(vl(i)).padStart(2, "0")} ${i.getFullYear()}`;
        break;
      case "month":
...
function Jt(t, s = /* @__PURE__ */ new Date()) {
  try {
  ...
  return (n) => {
    ...
    if (i && (i.classList.add("sg-renderer-tz-cell"), s && !i._sgTzBound)) {
      ...
      i._sgSelectOpts = d.map((p) => {
        const f = Jt(p), g = p.split("/").pop().replace(/_/g, " ");
        return { value: p, label: `${g} (${f || "?"}) — ${p}` };
      }), i._sgSelectClearable = !1, i.addEventListener("dblclick", (p) => {
        p._sgSelectHandled || (p._sgSelectHandled = !0, p.stopPropagation(), Fr(i, n));
      });
    }
    if (k(r)) return "";
    const o = String(r), a = Jt(o), l = t ? o.split("/").pop().replace(/_/g, " ") : o, c = u("span", { class: "sg-renderer-tz", title: o });
...
function Ks({} = {}) {
  return ({ value: t, td: s }) => {
    ...
    const e = String(t).trim(), n = xl(e), r = u("span", { class: "sg-renderer-cron" });
...
function Ws({
  ...
  return ({ value: c, td: d }) => {
    ...
    const f = (p - t) / Math.max(1e-9, s - t), g = r / 2 + 1, h = e / 2, y = n - g, m = Math.min(h - g, y - g), v = ($) => {
      if ($ <= 0) return "";
      const L = h - m, E = y, N = h, D = y - m;
      if ($ >= 1) {
        const F = h + m;
        return `M ${L},${E} A ${m},${m} 0 0 1 ${N},${D} A ${m},${m} 0 0 1 ${F},${E}`;
      }
      const I = Math.PI + Math.PI * $, B = h + m * Math.cos(I), H = y + m * Math.sin(I);
      return $ <= 0.5 ? `M ${L},${E} A ${m},${m} 0 0 1 ${B},${H}` : `M ${L},${E} A ${m},${m} 0 0 1 ${N},${D} A ${m},${m} 0 0 1 ${B},${H}`;
    }, b = v(1), w = v(f), C = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    C.setAttribute("viewBox", `0 0 ${e} ${n}`), C.setAttribute("width", e), C.setAttribute("height", n);
    const S = document.createElementNS("http://www.w3.org/2000/svg", "path");
    if (S.setAttribute("d", b), S.setAttribute("stroke", o), S.setAttribute("stroke-width", r), S.setAttribute("fill", "none"), S.setAttribute("stroke-linecap", "round"), C.append(S), w) {
      const $ = document.createElementNS("http://www.w3.org/2000/svg", "path");
      $.setAttribute("d", w), $.setAttribute("stroke", i), $.setAttribute("stroke-width", r), $.setAttribute("fill", "none"), $.setAttribute("stroke-linecap", "round"), C.append($);
    }
...
function qs({
  ...
  return ({ value: i, td: o }) => {
    ...
    let p = "";
    a.forEach((g, h) => {
      const y = typeof g == "number" ? g : g === "W" || g === "w" || g === "1" || g === !0 ? 1 : g === "L" || g === "l" || g === "-1" || g === !1 ? -1 : 0, m = h * l + c / 2, v = l - c;
      y > 0 ? p += `<rect x="${m}" y="0" width="${v}" height="${d - 1}" fill="${e}"/>` : y < 0 ? p += `<rect x="${m}" y="${d + 1}" width="${v}" height="${d - 1}" fill="${n}"/>` : p += `<rect x="${m}" y="${d - 0.5}" width="${v}" height="1" fill="${r}"/>`;
    });
...
function Ys({
  ...
  return ({ value: r, td: i }) => {
    ...
    let f = "";
    o.forEach((h, y) => {
      const m = y * d + p / 2, v = d - p, w = (Number(h.value) || 0) / l * s;
      f += `<rect x="${m}" y="${s - w}" width="${v}" height="${w}" fill="${h.color || e}"/>`, n && h.label && (f += `<text x="${m + v / 2}" y="${s - 1}" font-size="7" fill="#fff" text-anchor="middle">${String(h.label).slice(0, 3)}</text>`);
    });
...
function Zs({
  ...
  return ({ value: i, td: o }) => {
    ...
    let a = [];
    if (Array.isArray(i) && Array.isArray(i[0]) ? a = i.map((C, S) => ({ color: e[S % e.length], data: C })) : i && Array.isArray(i.series) ? a = i.series.map((C, S) => ({ color: C.color || e[S % e.length], data: C.data })) : Array.isArray(i) && (a = [{ color: e[0], data: i }]), !a.length) return "";
    const l = a.flatMap((C) => C.data.map(Number).filter(Number.isFinite)), c = Math.max(...l), d = Math.min(...l), p = Math.max(1e-9, c - d), f = 1.2, g = f, h = s - f, y = h - g;
    let m = "";
    for (const C of a) {
      const S = C.data.map((A, $) => {
        const L = $ / Math.max(1, C.data.length - 1) * t, E = h - (Number(A) - d) / p * y;
        return `${L.toFixed(2)},${E.toFixed(2)}`;
      });
      m += `<polyline fill="none" stroke="${C.color}" stroke-width="1.4" stroke-linejoin="round" stroke-linecap="round" points="${S.join(" ")}"/>`;
    }
    const v = u("span", { class: `sg-renderer-miniline${r ? " is-fill" : ""}` }), b = r ? "100%" : String(t), w = String(s);
    return v.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${t} ${s}" preserveAspectRatio="none" width="${b}" height="${w}">${m}</svg>`, v;
  };
}
const Sl = '<svg viewBox="0 0 12 12" aria-hidden="true"><path fill="currentColor" d="M6 2l4 6H2z"/></svg>', Ll = '<svg viewBox="0 0 12 12" aria-hidden="true"><path fill="currentColor" d="M6 10L2 4h8z"/></svg>', kl = '<svg viewBox="0 0 12 12" aria-hidden="true"><rect x="2" y="5" width="8" height="2" fill="currentColor"/></svg>';
function Xs({
  width: t = 60,
  height: s = 16,
  showValue: e = !0,
  ...
  return ({ value: r, td: i }) => {
    ...
    if (d.innerHTML = l === "up" ? Sl : l === "down" ? Ll : kl, c.append(d), e) {
    ...
    if (Array.isArray(o.series) && o.series.length) {
      const p = Math.max(...o.series), f = Math.min(...o.series), g = Math.max(1e-9, p - f), h = o.series.map((v, b) => {
        const w = b / Math.max(1, o.series.length - 1) * t, C = s - (Number(v) - f) / g * s;
        return `${w.toFixed(2)},${C.toFixed(2)}`;
      }).join(" "), y = l === "up" ? "#10b981" : l === "down" ? "#ef4444" : "#9ca3af", m = u("span", { class: "sg-renderer-trend-spark" });
      m.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${t} ${s}" width="${t}" height="${s}"><polyline fill="none" stroke="${y}" stroke-width="1.2" stroke-linejoin="round" stroke-linecap="round" points="${h}"/></svg>`, c.append(m);
    }
    return c;
  };
}
...
function Js({
  ...
  return ({ value: e, row: n, td: r }) => {
    ...
    const i = t || (n && s ? n[s] : null), o = El(e, i);
    return u(
      "span",
...
function $l(t, s) {
  ...
  o.addEventListener("mousedown", ($) => $.stopPropagation());
  const a = ($, L, E = {}) => {
    const N = u("label", { class: "sg-renderer-address-field" });
    N.append(u("span", { class: "sg-renderer-address-label" }, document.createTextNode($)));
    const D = u("input", { type: "text", class: "sg-renderer-address-input", ...E });
    return D.value = i[L] || "", D.dataset.key = L, N.append(D), { wrap: N, input: D };
  ...
  function m() {
    ...
    e && n?.field != null && (e[n.field] = L), $?.applyTransaction && $.applyTransaction({ update: [e] });
    const N = t.closest('[data-controller~="grid"]');
    N && N.dispatchEvent(new CustomEvent("grid:cellValueChanged", {
      bubbles: !0,
      detail: { rowId: e?.id ?? e?._sg_id, colId: n?.field, oldValue: E, newValue: L }
    })), ae();
  }
  const v = u("div", { class: "sg-renderer-textarea-footer" }), b = u(
    "span",
    { class: "sg-renderer-textarea-hint" },
  ...
  w.addEventListener("click", () => ae()), C.addEventListener("click", m), v.append(b, w, C), o.append(g, h, y, v), o.addEventListener("keydown", ($) => {
    $.key === "Enter" && $.target.tagName === "INPUT" ? ($.preventDefault(), m()) : $.key === "Escape" && ($.stopPropagation(), ae());
  });
  function S($) {
    $.key === "Escape" && ($.stopPropagation(), ae());
  }
  function A($) {
    !o.contains($.target) && !t.contains($.target) && ae();
  }
  document.addEventListener("keydown", S), setTimeout(() => document.addEventListener("mousedown", A), 0), document.body.appendChild(o), U(o, t), setTimeout(() => l.input.focus(), 0), Je = { pop: o, onKey: S, onDocClick: A, anchor: t };
}
function Tl(t) {
...
function Qs({ empty: t = "", editable: s = !1 } = {}) {
  return (e) => {
    const { value: n, td: r } = e;
    r && (r.classList.add("sg-renderer-address-cell"), s && !r._sgAddrBound && (r._sgAddrBound = !0, r._sgTextareaRows = 6, r._sgTextareaCols = 36, r.addEventListener("dblclick", (c) => {
      c._sgTextareaHandled || (c._sgTextareaHandled = !0, c.stopPropagation(), $l(r, e));
    })));
    const i = Tl(n);
    if (!i) return t;
    if (i._raw)
...
function ei({ empty: t = "", multiline: s = !1 } = {}) {
  return ({ value: e, td: n }) => {
...
function Rl(t) {
  ...
  return s.push(e % 103), s.push(Dl), s.map((n) => Nl[n]).join("") + "11";
}
function ti({
  height: t = 32,
  showText: s = !0,
  ...
  return ({ value: n, td: r }) => {
    ...
    const i = String(n), o = Rl(i), a = o.length * e, l = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${a} ${t}" width="${a}" height="${t}" aria-label="barcode ${i}">`;
    let c = 0, d = "";
...
function ni({} = {}) {
  return ({ value: t, td: s }) => {
    ...
    const e = String(t).replace(/\s+/g, "").toUpperCase(), n = /^[A-Z]{2}\d{2}[A-Z0-9]{1,30}$/.test(e), r = e.match(/.{1,4}/g)?.join(" ") || e, i = e.slice(0, 2), o = Tt[i];
    return u(
      "span",
...
function oi({} = {}) {
  return ({ value: t, td: s }) => {
    ...
    const e = String(t).replace(/\s+/g, "").toUpperCase(), n = /^[A-Z]{2}[A-Z0-9]{2,15}$/.test(e), r = e.slice(0, 2), i = Tt[r];
    return u(
      "span",
...
const Pl = /^[A-CEGHJ-PR-TW-Z][A-CEGHJ-NPR-TW-Z]\d{6}[A-D]$/i;
function ai({} = {}) {
  return ({ value: t, td: s }) => {
    if (k(t)) return "";
    s && s.classList.add("sg-renderer-nin-cell");
    const e = String(t).replace(/\s+/g, "").toUpperCase(), n = Pl.test(e), r = n ? `${e.slice(0, 2)} ${e.slice(2, 4)} ${e.slice(4, 6)} ${e.slice(6, 8)} ${e.slice(8)}` : e;
    return u(
      "span",
      { class: `sg-renderer-uuid${n ? "" : " is-invalid"}`, title: e },
      u("code", { class: "sg-renderer-uuid-mono" }, document.createTextNode(r))
...
function At(t, s = 24) {
  ...
  if (t.avatar)
    ...
    const n = t.name || t.label || "?", r = t.color || en[Il(n) % en.length];
    e.style.background = r, e.append(u(
      "span",
      { class: "sg-renderer-avatar-stack-initials" },
      document.createTextNode(Vl(n))
    ));
  }
  return e;
}
function li({
  ...
  return (n) => {
    ...
    for (const g of d) f.append(At(g, a));
...
function ci({
  ...
  return (e) => {
    ...
    const l = rt[a] || rt.offline, c = typeof n == "object" && n.label || l.label, d = u("span", { class: "sg-renderer-presence", title: c });
...
function di({
  ...
  return (o) => {
    const { value: a, td: l } = o, c = o?.col?.cellRendererConfig || {}, d = c.showPresence ?? t, p = c.showAvatar ?? s, f = c.size ?? e, g = c.editable ?? n, h = c.options ?? r, y = c.clearable ?? i;
    if (l && g && !l._sgAssigneeBound && (l._sgAssigneeBound = !0, l._sgAssigneeOpts = h || [], l._sgAssigneeClearable = y, l.addEventListener("dblclick", (C) => {
      C._sgAssigneeHandled || (C._sgAssigneeHandled = !0, C.stopPropagation(), Fl(l, o));
    })), k(a)) return u(
      "span",
    ...
    const b = u("span", { class: "sg-renderer-assignee" });
    p && b.append(At(m, f));
    const w = u(
      "span",
    ...
    if (d && m.presence) {
      const C = String(m.presence).toLowerCase(), S = rt[C] || rt.offline;
      w.prepend(u("span", {
...
function Fl(t, s) {
  ...
  function d(f) {
    f.key === "Escape" && (f.stopPropagation(), De());
  }
  function p(f) {
    !l.contains(f.target) && !t.contains(f.target) && De();
  }
  document.addEventListener("keydown", d), setTimeout(() => document.addEventListener("mousedown", p), 0), document.body.appendChild(l), U(l, t), Qe = { pop: l, onKey: d, onDocClick: p, anchor: t };
}
function ui({
  min: t = 0,
  max: s = 100,
  ...
  return (l) => {
    const { value: c, row: d, col: p, api: f, td: g } = l, h = l?.col?.cellRendererConfig || {}, y = h.min ?? t, m = h.max ?? s, v = h.step ?? e, b = h.range ?? o, w = n || ((E) => String(E)), C = h.showValue ?? a, S = h.color || r, A = h.editable ?? i;
    if (g && g.classList.add("sg-renderer-slider-cell"), k(c) && !b)
      return u(
        "span",
        { class: "sg-renderer-slider-placeholder" },
        document.createTextNode("—")
      );
    const $ = u("div", { class: "sg-renderer-slider" });
    function L(E) {
      const N = d && p?.field != null ? d[p.field] : null;
      ...
      D && D.dispatchEvent(new CustomEvent("grid:cellValueChanged", {
        bubbles: !0,
        detail: { rowId: d?.id ?? d?._sg_id, colId: p?.field, oldValue: N, newValue: E }
      }));
    }
    if (b) {
      let Z = function() {
        let O = Number(F.value), R = Number(X.value);
        O > R && ([O, R] = [R, O]);
        const G = (O - y) / D * 100, Q = (R - y) / D * 100;
        H.style.left = `${G}%`, H.style.width = `${Math.max(0, Q - G)}%`, ee.textContent = `${w(O)} – ${w(R)}`;
      }, te = function() {
        let O = Number(F.value), R = Number(X.value);
        O > R && ([O, R] = [R, O]), Z(), L([O, R]);
      };
      const [E, N] = Array.isArray(c) ? c : [y, m], D = Math.max(1, m - y), I = u("div", { class: "sg-renderer-slider-range-stack" }), B = u("div", { class: "sg-renderer-slider-range-rail" }), H = u("div", { class: "sg-renderer-slider-range-fill", style: `background:${S};` }), F = u("input", {
        type: "range",
      ...
      I.style.setProperty("--sg-slider-accent", S);
      const ee = u(
        "span",
        { class: "sg-renderer-slider-value" },
        document.createTextNode(`${w(E)} – ${w(N)}`)
      );
      [F, X].forEach((O) => {
        O.addEventListener("click", (R) => R.stopPropagation()), O.addEventListener("input", Z), O.addEventListener("change", te);
      }), I.append(B, H, F, X), $.append(I), C && $.append(ee), Z();
    } else {
      const E = Number(c), N = Number.isFinite(E) ? E : y, D = u("input", {
        type: "range",
...
function fi(t, { label: s = "exp" } = {}) {
  if (!t) return null;
  const e = Hl(t);
  if (e == null) return null;
  const n = Ol(e), r = t instanceof Date ? t : new Date(t), i = `${String(r.getMonth() + 1).padStart(2, "0")}/${r.getFullYear()}`, o = e < 0 ? `expired ${i}` : `${s} ${i}`, a = e < 0 ? `Expired ${Math.abs(e)} day${Math.abs(e) === 1 ? "" : "s"} ago` : e === 0 ? "Expires today" : `Expires in ${e} day${e === 1 ? "" : "s"}`;
  return u("span", {
    class: `sg-renderer-expiry ${n}`,
    title: a
  }, document.createTextNode(o));
}
function gi({ editable: t = !0 } = {}) {
  return (s) => {
    ...
    if (n && (n.classList.add("sg-renderer-licence-cell"), n._sgLicence = e, t && !n._sgLicenceEditBound && (n._sgLicenceEditBound = !0, n.addEventListener("dblclick", (o) => {
      o._sgLicenceHandled || (o._sgLicenceHandled = !0, o.stopPropagation(), hi(n, s));
    }))), k(e)) return "";
    if (typeof e == "string")
      return u(
        "span",
    ...
    e.state && r.append(pi(e.state)), e.number && r.append(u(
      "span",
...
function be() {
  if (!et) return;
  const { pop: t, onKey: s, onDocClick: e } = et;
  document.removeEventListener("keydown", s), document.removeEventListener("mousedown", e), t.remove(), et = null;
}
function hi(t, s) {
  be();
  ...
  function l(w, C, S) {
    const A = u("label", { class: "sg-licence-editor-field", "data-field": C });
    return A.append(u("span", { class: "sg-licence-editor-label" }, document.createTextNode(w))), A.append(S), A;
  }
  ...
  for (const w of Kn)
    c.append(u(
      ...
      document.createTextNode(`${w} — ${Lt[w]}`)
    ));
  const d = u("input", {
    type: "text",
    name: "number",
  ...
  g.append(h, y), o.append(a, g), r.append(i, o);
  function m() {
    const w = {
      state: c.value || "",
      number: d.value.trim(),
      class: p.value.trim(),
      expires: f.value || ""
    }, C = !w.state && !w.number && !w.class && !w.expires;
    Gl(t, s, C ? null : w), be();
  }
  ...
  function b(w) {
    !r.contains(w.target) && !t.contains(w.target) && be();
  }
  document.addEventListener("keydown", v), setTimeout(() => document.addEventListener("mousedown", b), 0), document.body.appendChild(r), U(r, t), d.focus(), d.select(), et = { pop: r, onKey: v, onDocClick: b };
}
function Gl(t, s, e) {
  const { row: n, col: r, api: i } = s, o = n && r?.field != null ? n[r.field] : null;
...
function Y({
  ...
  return (r) => {
    ...
    if (o && (o.classList.add("sg-renderer-licence-cell"), o._sgLicence = i, n && !o._sgLicenceEditBound && (o._sgLicenceEditBound = !0, o.addEventListener("dblclick", (d) => {
      d._sgLicenceHandled || (d._sgLicenceHandled = !0, d.stopPropagation(), hi(o, r));
    }))), k(i)) return "";
    ...
    i.state ? a.append(pi(i.state)) : t && a.append(u(
      "span",
      { class: "sg-renderer-compliance-prefix" },
      document.createTextNode(t)
    )), i.number && a.append(u(
    ...
    const c = fi(i.expires, { label: e });
    return c && a.append(c), a;
  };
}
function mi(t = {}) {
  return Y({ prefix: "CIC", classLabel: "White Card", ...t });
}
function bi(t = {}) {
  return Y({ prefix: "BC", classLabel: "Blue Card (QLD)", ...t });
}
function yi(t = {}) {
  return Y({ prefix: "WWCC", ...t });
}
function wi(t = {}) {
  return Y({ prefix: "HRWL", ...t });
}
function _i(t = {}) {
  return Y({ prefix: "COES", classLabel: "Electrical Safety", ...t });
}
function vi(t = {}) {
  return Y({ prefix: "COC", classLabel: "Compliance", ...t });
}
function Ci(t = {}) {
  return Y({ prefix: "QBCC", ...t });
}
function xi(t = {}) {
  return Y({ prefix: "VBA", ...t });
}
function Si(t = {}) {
  return Y({ prefix: "Gas", classLabel: "Type A", ...t });
}
function Li(t = {}) {
  return Y({ prefix: "Asbestos", classLabel: "Class B", ...t });
}
function ki(t = {}) {
  return Y({ prefix: "ARC RHL", ...t });
}
function Ei(t = {}) {
  return Y({ prefix: "PSC", classLabel: "Pool Safety", ...t });
}
function $i(t = {}) {
  return Y({ prefix: "T&T", expiryLabel: "next", ...t });
}
_("email", gn());
_("url", hn());
_("phone", mn());
...
_("empty", Rr());
_("credit-card", Pr());
_("loading-shimmer", Ir());
_("audio-attachment", nr());
_("select", Vr());
_("multiselect", Hr());
_("combobox", Or());
_("slider", ui());
...
_("postal-code", Js());
_("address-us", Qs());
_("address-generic", ei());
_("barcode", ti());
_("gauge", Ws());
_("win-loss", qs());
_("mini-bar-chart", Ys());
_("mini-line-chart", Zs());
_("trend", Xs());
_("countdown", Gs());
_("age", zs());
_("fiscal-period", js());
...
_("blue-card", bi());
_("wwcc", yi());
_("high-risk-licence", wi());
_("coes", _i());
_("coc", vi());
_("qbcc-licence", Ci());
_("vba-licence", xi());
_("gas-certificate", Si());
_("asbestos-licence", Li());
_("refrigerant-licence", ki());
_("pool-safety-cert", Ei());
_("test-and-tag", $i());
const M = {
...
T("email", { copy: M.text.copy, parse: (t) => String(t ?? "").trim() });
T("url", { copy: M.text.copy, parse: (t) => String(t ?? "").trim() });
T("phone", M.digits);
T("currency", M.number);
T("percent", {
  copy: M.number.copy,
  parse: (t) => it(String(t ?? "").replace(/%$/, ""))
});
T("progress-bar", M.number);
T("star-rating", M.number);
T("tags", M.stringList);
T("country-flag", {
...
T("avatar", M.text);
T("date", M.date);
T("datetime", M.datetime);
T("relative-time", M.datetime);
T("duration", {
  copy: M.number.copy,
...
T("truncate", M.text);
T("copyable", M.text);
T("image", { copy: M.text.copy, parse: (t) => String(t ?? "").trim() });
T("color-swatch", { copy: M.text.copy, parse: (t) => String(t ?? "").trim() });
T("sparkline", M.numberList);
T("heatmap-cell", M.number);
T("mask", M.text);
T("highlight", M.text);
T("multi-line", M.text);
T("attachments", {
...
T("timeline-steps", M.text);
T("mention", M.text);
T("expand", M.text);
T("units", M.number);
T("ip-address", { copy: M.text.copy, parse: (t) => String(t ?? "").trim() });
T("bsb", M.digits);
T("acn", M.digits);
T("tfn", M.digits);
T("medicare", M.digits);
T("audio", { copy: M.text.copy, parse: (t) => String(t ?? "").trim() });
T("video", { copy: M.text.copy, parse: (t) => String(t ?? "").trim() });
T("reactions", M.json);
T("comment-count", {
  copy: ({ value: t }) => {
    if (t == null || t === "") return "";
...
T("plural", M.number);
T("empty", M.text);
T("credit-card", M.digits);
T("loading-shimmer", M.text);
T("audio-attachment", {
...
T("color-picker", { copy: M.text.copy, parse: (t) => String(t ?? "").trim() });
T("textarea", M.text);
T("action-button", M.text);
T("menu", M.text);
T("split-button", M.text);
T("row-actions", M.text);
T("trade-licence", M.json);
T("white-card", M.json);
T("blue-card", M.json);
T("wwcc", M.json);
T("high-risk-licence", M.json);
T("coes", M.json);
T("coc", M.json);
T("qbcc-licence", M.json);
T("vba-licence", M.json);
T("gas-certificate", M.json);
T("asbestos-licence", M.json);
T("refrigerant-licence", M.json);
T("pool-safety-cert", M.json);
T("test-and-tag", M.json);
const zl = {
  email: gn,
  url: hn,
  phone: mn,
  ...
  gasCertificate: Si,
  asbestosLicence: Li,
  refrigerantLicence: ki,
  poolSafetyCert: Ei,
  testAndTag: $i
}, jl = 32, tn = 100, Re = '<svg viewBox="0 0 640 640" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path fill="currentColor" d="M471.1 297.4C483.6 309.9 483.6 330.2 471.1 342.7L279.1 534.7C266.6 547.2 246.3 547.2 233.8 534.7C221.3 522.2 221.3 501.9 233.8 489.4L403.2 320L233.9 150.6C221.4 138.1 221.4 117.8 233.9 105.3C246.4 92.8 266.7 92.8 279.2 105.3L471.2 297.3z"/></svg>', Ul = '<svg viewBox="0 0 640 640" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path fill="currentColor" d="M64 157.7C64 141.3 77.3 128 93.7 128L546.4 128C562.8 128 576.1 141.3 576.1 157.7C576.1 165.6 573 173.1 567.4 178.7L400 345.9L400 546.3C400 562.7 386.7 576 370.3 576C362.4 576 354.9 572.9 349.3 567.3L247 465C242.5 460.5 240 454.4 240 448L240 345.9L72.7 178.6C67.1 173.1 64 165.5 64 157.7zM137.9 176L281 319C285.5 323.5 288 329.6 288 336L288 438.1L352 502.1L352 336C352 329.6 354.5 323.5 359 319L502 176L137.9 176z"/></svg>', Kl = /* @__PURE__ */ new Set([
  "number",
  "currency",
  "percent",
  "compactNumber",
...
class Nt extends ie {
  constructor() {
    ...
    V(this, "_onCopy", (e) => {
      ...
      if (!i.length) return;
      const a = i.length === 1 && i[0].length === 1 ? String(i[0][0] ?? "") : i.map((l) => l.map((c) => ql(c)).join("	")).join(`
`);
    ...
    V(this, "_onPaste", (e) => {
      ...
      for (let h = 0; h < l; h++) {
        ...
        for (let b = 0; b < c; b++) {
          ...
          const C = p[w];
          if (!C) continue;
          if (!C.editable || C._isCheckbox || C._isRowNumber || C._isGroupCol || C._isMasterExpand || C._isSpacer) {
            f.push({ rowId: this._rowId(m), colId: C.field || "", reason: "not-editable" });
            continue;
          }
          const S = a ? v[0] : v[b] ?? "", A = this._parsePasteValue(S, m, C);
          if (A === void 0) {
            f.push({ rowId: this._rowId(m), colId: C.field, reason: "parse-failed", text: S });
            continue;
          }
          const $ = m[C.field];
          A !== $ && (m[C.field] = A, g = !0, P(this.element, "grid:cellValueChanged", {
            rowId: this._rowId(m),
            colId: C.field,
            oldValue: $,
            newValue: A,
  ...
  initialize() {
    this.state = {
      ...
      editing: null,
      pagination: { enabled: !1, page: 0, pageSize: tn },
      scrollTop: 0,
      viewportHeight: 400,
  ...
  connect() {
    ...
    }, this._captureInitialMarkup(), this._buildChrome(), this.element.gridApi = to(this), queueMicrotask(() => this._initialLoad());
  }
  disconnect() {
  ...
  _openFallbackFilterPopover(e, n) {
    ...
    const l = e.filter === "number" ? "number" : e.filter === "date" ? "date" : "text", c = x("input", { type: l, value: r.value ?? "" }), d = x("input", { type: l, value: r.value2 ?? "", style: { display: "none" } }), p = () => {
      const m = a.value, v = m === "inRange", b = !(m === "blank" || m === "notBlank");
      c.style.display = b ? "" : "none", d.style.display = v ? "" : "none";
    };
  ...
  registerColumn(e, n) {
    ...
    if (r >= 0) {
      ...
      if (c._headerEl === n && Zl(c, l)) return;
  ...
  unregisterColumn(e) {
    this._thead?.querySelector(`th[data-header-cell-field-value="${le(e)}"]`) || (this.state.columnDefs = this.state.columnDefs.filter((n) => n.field !== e), this.scheduleRender("columns"));
  }
  // ----- Sort -----
  toggleSort(e, n = !1) {
  ...
  filteredCount() {
    ...
    let r = on(this.state.rowData, this.state.filterModel, e);
    return r = an(r, this.state.quickFilter, n), r.length;
  }
  ...
  stopEditing(e = !1) {
    if (!this.state.editing) return;
    const { rowId: n, colId: r, originalValue: i, draftValue: o } = this.state.editing, a = this._tbody.querySelector(`tr[data-row-id="${le(n)}"] td[data-col-id="${le(r)}"]`);
    let l = i;
    if (!e && a) {
      ...
      d ? l = Jl(d.value, this._colByField(r)?.type) : o !== void 0 && (l = o);
    }
  ...
  autoSizeColumn(e) {
    ...
    if (!n) return;
    const r = le(e), i = this._thead?.querySelector(
...
`);
  ...
  _render() {
    const e = this._dirty;
    this._dirty = /* @__PURE__ */ new Set(), (e.has("data") || e.has("filter") || e.has("sort") || e.has("page") || e.has("group") || e.has("pivot") || e.has("tree") || e.size === 0) && (this._displayList = Qi({
  ...
  _renderHeader() {
    ...
    const e = this._visibleCols(), n = Xi(e, this._headerLayoutOpts());
  ...
  _renderHeaderSingleRow(e) {
    ...
    for (const f of e) {
      const g = n.querySelector(`th[data-header-cell-field-value="${le(f.field)}"]`) || n.querySelector(`th[data-field="${le(f.field)}"]`);
  ...
  _columnAlignment(e) {
    ...
    return typeof n == "string" && Kl.has(n) ? "right" : null;
  }
  _ensureHeaderChrome(e, n, r) {
    ...
    if (n.sortable)
      if (o || (o = x("span", { class: "sg-sort-icon", "aria-hidden": "true" }), o.innerHTML = Re, i.appendChild(o)), r && this.state.sortModel.length > 1) {
    ...
    n.filter ? a || (a = x("span", {
      ...
      title: "Filter"
    }), a.innerHTML = Ul, i.appendChild(a)) : a && a.remove(), n.resizable !== !1 && !e.querySelector(".sg-resize-handle") && !n._isCheckbox && e.appendChild(x("span", {
  ...
  _renderBody() {
    ...
    let i = n, o = 0;
    if (r) {
      const f = this._viewport?.clientHeight || 400, g = this.state.rowHeight, h = eo(this.state.scrollTop, f, g, n.length, 8);
    ...
    if (r) {
      ...
      l.appendChild(this._spacerRow(g, e.length)), i.forEach((y) => l.appendChild(this._buildRow(y, e, a, p(y)))), l.appendChild(this._spacerRow(h, e.length));
    } else
  ...
  _buildPinnedBottomRow(e) {
    ...
    for (const a of e) {
      if (a._isSpacer) {
        n.appendChild(x("td", { class: "sg-spacer-cell", "aria-hidden": "true" }));
        continue;
      }
  ...
  _buildSeparatorRow(e, n, r) {
    ...
    for (const p of n) {
      if (p._isSpacer) {
        o.appendChild(x("td", { class: "sg-spacer-cell", "aria-hidden": "true" }));
        continue;
      }
      if (l(p)) {
  ...
  _renderRow(e, n, r, i) {
    ...
    for (let h = 0; h < r.length; h++) {
      ...
      const m = y._isRowNumber || y._isCheckbox || y._isGroupCol || y._isMasterExpand || y._isSpacer;
      if (y._isSpacer) {
        const A = x("td", { class: "sg-spacer-cell", "aria-hidden": "true" });
        e.appendChild(A);
        continue;
      }
      const v = f && !m ? Number(f[y.field]) : 0, b = Math.max(1, Math.min(v || 1, r.length - h));
      b > 1 && (g = b - 1);
      const w = `${l}:${y.field}`, C = x("td", {
        "data-col-id": y.field,
      ...
      if (b > 1 && C.classList.add("sg-merged-cell"), y.type === "number" && C.classList.add("sg-renderer-number"), y.pinned === "left" ? C.style.left = o.left[y.field] + "px" : y.pinned === "right" && (C.style.right = o.right[y.field] + "px"), y._isRowNumber) {
      ...
      if (y._isCheckbox) {
        ...
        const A = x("input", { type: "checkbox" });
        A.checked = this.state.selection.has(this._rowId(n)), C.appendChild(A), e.appendChild(C);
        continue;
      }
      if (y._isGroupCol) {
      ...
      if (y._isMasterExpand) {
        ...
        const A = this._isDetailExpanded(this._rowId(n)), $ = x("span", {
          class: "sg-master-expand-caret",
          "data-expanded": A ? "true" : "false",
          "aria-hidden": "true"
        });
        $.innerHTML = Re, C.appendChild($), e.appendChild(C);
        continue;
      }
      if (this.state.editing && this.state.editing.rowId === this._rowId(n) && this.state.editing.colId === y.field) {
        ...
        const A = this.state.editing.initialValue !== void 0 ? this.state.editing.initialValue : z(n, y), { node: $, control: L } = this._buildEditor(y, A);
        C.appendChild($);
        const E = this.state.editing.initialValue !== void 0;
        queueMicrotask(() => {
          if (L?.focus(), E || L?.select?.(), L?.type && Wl.has(L.type))
            try {
              L.showPicker?.();
            } catch {
            }
        ...
        this._renderCellContent(C, n, y, i);
      d && y.field === p && this._decorateTreeCell(C, d), e.appendChild(C);
    }
  }
  ...
  _decorateTreeCell(e, n) {
    if (e.classList.add("sg-tree-cell"), e.setAttribute("data-tree-level", String(n.level)), e.style.paddingLeft = `${8 + n.level * 18}px`, n.hasChildren) {
      ...
      r.innerHTML = Re, e.insertBefore(r, e.firstChild);
    } else {
  ...
  _renderCellContent(e, n, r, i = null) {
    if (r.cellRenderer) {
      const o = Vt(r.cellRenderer);
      if (o) {
        const l = z(n, r), c = ne(n, r);
      ...
      const a = ve(r.cellRenderer);
      if (typeof a == "function") {
        const l = z(n, r), c = ne(n, r), d = a({ value: l, row: n, col: r, td: e, formatted: c, rowNum: i, api: this.element.gridApi });
        if (d == null) return;
        if (typeof d == "string") {
          e.innerHTML = d;
          return;
  ...
  _setupPersistence() {
    ...
    for (const n of nn) this.element.addEventListener(n, e);
  ...
  _teardownPersistence() {
    if (this._persistListener) {
      for (const e of nn) this.element.removeEventListener(e, this._persistListener);
  ...
  _renderGroupRow(e, n, r) {
    ...
    for (const g of r) {
      if (g._isSpacer) {
        e.appendChild(x("td", { class: "sg-spacer-cell", "aria-hidden": "true" }));
        continue;
      }
      ...
      if (l || a ? g._isGroupCol : g.field === p) {
        if (h.classList.add("sg-group-cell"), h.style.paddingLeft = `${8 + f * 18}px`, !c) {
          ...
          m.innerHTML = Re, h.appendChild(m);
        }
        h.append(
  ...
  getRangeAggregates() {
    return this.state.cellSel.ranges.length ? Gi(this._cellRangeRawValues()) : null;
  }
  ...
  _copyCellValue(e, n) {
    const r = z(e, n), i = ne(e, n);
    if (n.cellRenderer) {
      const o = ve(n.cellRenderer);
  ...
  _moveActiveCell(e, n, r) {
    ...
    if (d < 0 && (d = c()), !(d < 0)) {
      if (p < 0 && (p = 0), r && this.state.cellSel.ranges[this.state.cellSel.activeIdx]) {
        const f = this.state.cellSel.ranges[this.state.cellSel.activeIdx], g = a(i.findIndex((y) => this._rowId(y) === f.focus.rowId) + e, 0, i.length - 1), h = a(o.findIndex((y) => y.field === f.focus.colId) + n, 0, o.length - 1);
...
V(Nt, "values", {
  ...
  pageSize: { type: Number, default: tn },
  rowHeight: { type: Number, default: jl },
...
class Mt extends ie {
  ...
  connect() {
    if (this.grid = no(this.element, "grid", this.application), !!this.grid) {
...
V(st, "outlets", ["grid"]), V(st, "targets", ["first", "prev", "next", "last", "pageInfo", "pageSize"]);
const mt = ["sum", "avg", "count", "min", "max"], Ql = '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path fill="currentColor" d="M3 3h5v18H3V3zm6.5 0h5v18h-5V3zM16 3h5v18h-5V3z"/></svg>', ec = '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path fill="currentColor" d="M8 6h2v2H8V6zm6 0h2v2h-2V6zM8 11h2v2H8v-2zm6 0h2v2h-2v-2zM8 16h2v2H8v-2zm6 0h2v2h-2v-2z"/></svg>';
class Mi extends ie {
  connect() {
  ...
  _build() {
    ...
    this._columnsTab = x("button", {
      ...
      "aria-pressed": "true",
      title: "Columns"
    }), this._columnsTab.innerHTML = Ql, this._columnsTab.addEventListener("click", () => this._onTabClick("columns")), s.appendChild(this._columnsTab), this.element.append(this._content, s);
  }
  _onTabClick(s) {
  ...
  _renderValueChip(s, e) {
    ...
    return o.addEventListener("click", (a) => {
      a.stopPropagation();
      const l = mt.indexOf(e), c = mt[(l === -1 ? 0 : l + 1) % mt.length];
...
function tc(t) {
  const s = t ?? Pi.start();
  return s.register("grid", Nt), s.register("header-cell", Mt), s.register("row", Ti), s.register("cell", Ai), s.register("filter", Ni), s.register("pagination", st), s.register("side-panel", Mi), s;
}
const nc = {
  start: tc,
  GridController: Nt,
  ...
  CellController: Ai,
  FilterController: Ni,
  PaginationController: st,
  SidePanelController: Mi,
  registerRenderer: _,
  getRenderer: ve,
  listRenderers: bo,
  renderers: zl
};
typeof window < "u" && !window.__stimulusGridStarted && (window.__stimulusGridStarted = !0, window.StimulusGrid = nc);
export {
  Ai as CellController,
  Ni as FilterController,
  Nt as GridController,
  Mt as HeaderCellController,
  st as PaginationController,
  Ti as RowController,
  Mi as SidePanelController,
  nc as default,
  ve as getRenderer,
  bo as listRenderers,
