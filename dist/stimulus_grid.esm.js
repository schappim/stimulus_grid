...
var En = (n, i, e) => i in n ? Mn(n, i, { enumerable: !0, configurable: !0, writable: !0, value: e }) : n[i] = e;
var N = (n, i, e) => En(n, typeof i != "symbol" ? i + "" : i, e);
import { Controller as q, Application as Rn } from "@hotwired/stimulus";
function I(n, i) {
...
}, $n = { text: tt, number: Tn, date: Dn, boolean: kn, set: Nn };
function nt(n, i, e) {
  if (!e) return !0;
  const t = e.filterType || i.filter || "text", r = ($n[t] || tt)[e.type];
  if (!r) return !0;
  const o = I(n, i);
...
function In(n, i, e) {
  ...
  const t = (l, d) => {
    for (const { colId: c, sort: u } of i) {
      const p = e[c];
      if (!p) continue;
      const h = I(l, p), g = I(d, p), _ = typeof p.comparator == "function" ? p.comparator(h, g, l, d) : j(h, g, p.type);
...
function Bn(n, i, e) {
  ...
  for (const r of n)
    for (const o of i) {
      const a = ot(r, o, e), l = e.map((c) => r[c.field] == null ? "(Blank)" : String(r[c.field])).join(" · "), d = s ? l : `${l} · ${o.aggFunc}(${o.col.field})`;
      t.push({
        field: a,
        headerName: d,
        type: "number",
...
function Be(n, i, e, t) {
  ...
  for (const o of i) {
    ...
    for (const d of e) {
      const c = ot(o, d, t);
      s[c] = l.length ? rt(d.aggFunc, l, d.col) : null;
    }
  }
  return s;
}
function On({ rows: n, rowGroupCols: i = [], pivotCols: e, valueConfigs: t, isExpanded: s = () => !0, sortModel: r = [] }) {
  const o = Pn(n, e), a = Bn(o, t, e), l = {
    __sgGroup: !0,
    __pivotAll: !0,
    level: -1,
    field: null,
  ...
  const d = (h, g, _) => {
    ...
    const b = Array.from(S.values()).map(({ value: w, rows: x }) => {
      const L = w == null ? "" : String(w), T = _ ? `${_}|${y.field}=${L}` : `${y.field}=${L}`;
      return {
        __sgGroup: !0,
        level: g,
        field: y.field,
...
function zn(n, { pivotCols: i = [], valueConfigs: e = [], columnGroups: t = null } = {}) {
...
function qn(n, i = {}) {
  ...
  const e = n.map((r) => zn(r, i).slice()), t = Math.max(1, ...e.map((r) => r.length)), s = [];
  for (let r = 0; r < t; r++) {
    const o = [];
    let a = 0;
    for (; a < e.length; ) {
...
function Kn({
  ...
  const h = [], g = /* @__PURE__ */ new Map(), _ = (y, S, b, v) => {
    ...
    for (const x of w) {
      ...
      const T = l.get(L) || [], C = v || (c ? !!c.get(L) : !1), E = c ? T.filter((F) => C || u.get(o(F))) : T, R = E.length > 0, $ = R && (c ? !0 : !!r(L, S));
...
function Un(n) {
  ...
  if (n.treeData && !n.pivotMode && t.length === 0) {
    const c = n.treeParentField || "parent_id", u = Object.entries(n.filterModel || {}).filter(([, x]) => x != null), p = n.quickFilter ? String(n.quickFilter).toLowerCase() : "", g = u.length > 0 || p !== "" ? (x) => {
      ...
      for (const { colId: T, sort: C } of _) {
        const E = i[T];
        if (!E) continue;
        const R = I(x, E), $ = I(L, E), F = typeof E.comparator == "function" ? E.comparator(R, $, x, L) : j(R, $, E.type);
        if (F !== 0) return C === "desc" ? -F : F;
      }
      return 0;
    } : null, S = n.getRowId || ((x) => x?.id), { displayList: b, treeMeta: v } = Kn({
      rows: n.rowData,
      parentField: c,
      getRowId: S,
      passesFilter: g,
  ...
  let s = n.rowData;
  s = st(s, n.filterModel, i), s = it(s, n.quickFilter, e), s = In(s, n.sortModel, i);
  ...
  if (n.pivotMode && o.length && a.length) {
    const c = r.map((S) => i[S]), u = o.map((S) => i[S]), { columns: p, displayList: h, tree: g, combos: _ } = On({
      rows: s,
      rowGroupCols: c,
      pivotCols: u,
      valueConfigs: a,
  ...
  if (r.length) {
    const c = r.map((g) => i[g]), { displayList: u, tree: p } = Fn(
      s,
      c,
      i,
      n.aggModel,
...
(function() {
  let i = 1;
  for (let e = 0; e < 255; e++)
    ce[e] = i, Ae[i] = e, i <<= 1, i & 256 && (i ^= 285);
  for (let e = 255; e < 512; e++) ce[e] = ce[e - 255];
})();
function Le(n, i) {
  return n === 0 || i === 0 ? 0 : ce[Ae[n] + Ae[i]];
}
function Zn(n) {
...
function ts(n) {
  ...
  const u = Math.floor(t / r), p = t - u * r, h = [], g = Zn(s);
  let _ = 0;
  for (let C = 0; C < r; C++) {
    const E = C < r - p ? u : u + 1, R = d.slice(_, _ + E);
    _ += E, h.push({ data: R, ecc: Jn(R, g) });
  }
  const y = [], S = u + 1;
  for (let C = 0; C < S; C++)
  ...
  ns(v, w, b), ss(v, w, b), rs(v, w, b, e), e >= 7 && os(v, w, b, e), as(v, w, b, y);
  let x = 0, L = 1 / 0;
  const T = new Uint8Array(v);
  for (let C = 0; C < 8; C++) {
    T.set(v), je(T, w, b, C), ze(T, b, C);
    const E = ls(T, b);
    E < L && (L = E, x = C);
  }
  return je(v, w, b, x), ze(v, b, x), { size: b, matrix: v };
}
function V(n, i, e, t, s) {
  n[t * i + e] = s ? 1 : 0;
}
function ns(n, i, e) {
  ...
  for (const [s, r] of t)
    for (let o = -1; o <= 7; o++)
      for (let a = -1; a <= 7; a++) {
        ...
        a >= 2 && a <= 4 && o >= 2 && o <= 4);
        V(n, e, l, d, u), i[d * e + l] = 1;
      }
  for (let s = 0; s < 9; s++)
    i[s * e + 8] = 1, i[8 * e + s] = 1;
  for (let s = 0; s < 8; s++)
    i[(e - 1 - s) * e + 8] = 1, i[8 * e + (e - 1 - s)] = 1;
  V(n, e, 8, e - 8, 1), i[(e - 8) * e + 8] = 1;
}
function ss(n, i, e) {
  for (let t = 8; t < e - 8; t++)
    V(n, e, t, 6, t % 2 === 0), V(n, e, 6, t, t % 2 === 0), i[6 * e + t] = 1, i[t * e + 6] = 1;
}
const is = [
  null,
  [],
...
function os(n, i, e, t) {
  ...
  for (let a = 0; a < 18; a++) {
    ...
    V(n, e, d, c, l), i[c * e + d] = 1, V(n, e, c, d, l), i[d * e + c] = 1;
  }
}
function ze(n, i, e) {
  const t = Qn[e];
  for (let s = 0; s < 15; s++) {
    const r = (t >>> s & 1) === 1;
    s < 6 ? V(n, i, 8, s, r) : s < 8 ? V(n, i, 8, s + 1, r) : s < 9 ? V(n, i, 7, 8, r) : V(n, i, 14 - s, 8, r), s < 8 ? V(n, i, i - 1 - s, 8, r) : V(n, i, 8, i - 15 + s, r);
  }
  V(n, i, 8, i - 8, 1);
}
function as(n, i, e, t) {
...
const k = (n) => n == null || n === "", ps = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
function dt() {
  return ({ value: n }) => {
    if (k(n)) return "";
    const i = String(n);
    return ps.test(i) ? f("a", {
...
function ut({ defaultRegion: n = "AU" } = {}) {
  return ({ value: i }) => {
    if (k(i)) return "";
...
function pt({ currency: n = "USD", locale: i = "en-US", decimals: e } = {}) {
  return ({ value: t, td: s }) => {
...
function ft({ locale: n = void 0, dateStyle: i = "medium", ...e } = {}) {
...
function gt({ locale: n = void 0, dateStyle: i = "medium", timeStyle: e = "short", ...t } = {}) {
...
function mt({ locale: n = void 0, numeric: i = "auto", style: e = "long" } = {}) {
...
function _t({ unit: n = "ms", style: i = "compact" } = {}) {
  const e = hs[n] ?? 1;
  return ({ value: t, td: s }) => {
    if (s && s.classList.add("sg-renderer-number"), k(t)) return "";
    const r = Number(t) * e;
...
function bt({ locale: n = void 0, decimals: i, ...e } = {}) {
  const t = { ...e };
...
function yt({ locale: n = void 0, compactDisplay: i = "short", maximumFractionDigits: e = 1 } = {}) {
...
function vt({ binary: n = !0, decimals: i = 1, locale: e = void 0 } = {}) {
...
const gs = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335.1 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z"/></svg>', ms = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM175 175c9.4-9.4 24.6-9.4 33.9 0l47 47 47-47c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9l-47 47 47 47c9.4 9.4 9.4 24.6 0 33.9s-24.6 9.4-33.9 0l-47-47-47 47c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l47-47-47-47c-9.4-9.4-9.4-24.6 0-33.9z"/></svg>';
function wt({
  truthy: n = Te,
  nullLabel: i = "—",
  falseStyle: e = "icon"
  ...
  return ({ value: t }) => {
    ...
    if (n(t)) {
      ...
      return r.innerHTML = gs, r;
    }
    if (e === "hidden") return "";
    const s = f("span", { class: "sg-renderer-bool is-false", "aria-label": "false" });
    return s.innerHTML = ms, s;
  };
}
const _s = '<svg viewBox="0 0 384 512" aria-hidden="true"><path fill="currentColor" d="M214.6 41.4c-12.5-12.5-32.8-12.5-45.3 0l-160 160c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L160 141.2 160 448c0 17.7 14.3 32 32 32s32-14.3 32-32l0-306.7L329.4 246.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3l-160-160z"/></svg>', bs = '<svg viewBox="0 0 384 512" aria-hidden="true"><path fill="currentColor" d="M169.4 470.6c12.5 12.5 32.8 12.5 45.3 0l160-160c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L224 370.7 224 64c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 306.7L54.6 265.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l160 160z"/></svg>', ys = '<svg viewBox="0 0 448 512" aria-hidden="true"><path fill="currentColor" d="M64 256a32 32 0 1 0 0-64H384V160c0-12.9 7.8-24.6 19.8-29.6s25.7-2.2 34.9 6.9l96 96c12.5 12.5 12.5 32.8 0 45.3l-96 96c-9.2 9.2-22.9 11.9-34.9 6.9S384 364.9 384 352V320H64z"/></svg>';
function Ct({
  style: n = "percent",
  ...
  return n === "currency" ? o = new Intl.NumberFormat(e, {
    ...
    let c = "is-flat", u = ys;
    const p = !s;
    d > 0 ? (c = p ? "is-up" : "is-down", u = _s) : d < 0 && (c = p ? "is-down" : "is-up", u = bs);
...
function At({ position: n = "after" } = {}) {
  return ({ value: i }) => {
    ...
    return r.innerHTML = ge, r.addEventListener("click", async (o) => {
      ...
      try {
        navigator.clipboard?.writeText ? await navigator.clipboard.writeText(e) : Lt(e), r.innerHTML = St, r.classList.add("is-copied"), setTimeout(() => {
...
function Rt({
  ...
  return ({ value: a }) => {
    ...
    const d = s ?? Math.min(...l), u = Math.max(...l, s ?? -1 / 0) - d || 1, p = 1.5, h = 2.5, g = i - p * 2, _ = e - h * 2, y = (v) => p + (l.length === 1 ? g / 2 : v / (l.length - 1) * g), S = (v) => h + _ - (v - d) / u * _;
    let b = "";
    if (n === "bar") {
      const w = Math.max(1, (g - (l.length - 1) * 1) / l.length);
      for (let x = 0; x < l.length; x++) {
        const L = l[x], T = p + x * (w + 1), C = S(L), E = h + _ - C;
        b += `<rect x="${T.toFixed(2)}" y="${C.toFixed(2)}" width="${w.toFixed(2)}" height="${E.toFixed(2)}" fill="${o}"/>`;
      }
    } else {
      let v = "";
...
function kt({
  ...
  return ({ value: a, td: l }) => {
    ...
    const u = c * (o.length - 1), p = Math.min(o.length - 2, Math.floor(u)), h = u - p, g = Cs(o[p], o[p + 1], h);
    return l && (l.style.backgroundColor = ws(...g), l.style.color = Dt(g) ? "#111827" : "#ffffff"), s ? typeof r == "function" ? r(a) : String(a) : "";
  };
}
const xs = {
...
function Nt({
  ...
  const r = n ? xs[n] : null, o = n ? As.has(n) : !1, a = s === "right" || s !== "left" && o;
  return ({ value: l, td: d }) => {
...
function $t({
  ...
  return ({ value: t, api: s }) => {
    ...
    return o ? Ls(r, o, i, e) : document.createTextNode(r);
  };
}
function Ls(n, i, e, t) {
...
function It({ lines: n = null, separator: i = `
` } = {}) {
  return ({ value: e, td: t }) => {
    if (k(e)) return "";
...
}, Vt = '<svg viewBox="0 0 448 512" aria-hidden="true"><path fill="currentColor" d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z"/></svg>', ke = '<svg viewBox="0 0 384 512" aria-hidden="true"><path fill="currentColor" d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"/></svg>', Es = '<svg viewBox="0 0 320 512" aria-hidden="true"><path fill="currentColor" d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L77.3 256 214.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160z"/></svg>', Rs = '<svg viewBox="0 0 320 512" aria-hidden="true"><path fill="currentColor" d="M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-160 160c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 105.4 118.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l160 160z"/></svg>', Ts = /* @__PURE__ */ new Set(["mp3", "wav", "flac", "m4a", "ogg", "aac", "opus"]), Ds = /* @__PURE__ */ new Set(["mp4", "mov", "webm", "mkv", "avi", "m4v"]);
function Ft(n) {
  const i = String(n?.content_type || "").toLowerCase(), e = String(n?.filename || "").split(".").pop()?.toLowerCase() || "";
  return i.includes("pdf") || e === "pdf" ? "pdf" : i.startsWith("audio/") || Ts.has(e) ? "audio" : i.startsWith("video/") || Ds.has(e) ? "video" : i.includes("zip") || ["zip", "tar", "gz", "7z", "rar"].includes(e) ? "zip" : i.includes("sheet") || i.includes("excel") || i.includes("csv") || ["xls", "xlsx", "csv", "numbers"].includes(e) ? "sheet" : i.includes("word") || i.includes("document") || ["doc", "docx", "txt", "md", "rtf"].includes(e) ? "doc" : ["js", "ts", "rb", "py", "go", "rs", "java", "json", "xml", "html", "css", "sh", "sql"].includes(e) ? "code" : "file";
}
function ye(n) {
  if (n == null || n === "") return [];
  let i = n;
...
function Pt({
  ...
  return (d) => {
    ...
    if (y.forEach((b) => _.append(ks(b, n, g, o))), S > 0) {
      const b = f(
        "span",
    ...
    if (t) {
      ...
      }), _.append(b), Ns(u, d, { onUpload: a }), u.addEventListener("dblclick", (v) => {
...
function ks(n, i, e, t) {
  ...
  if (z(n) && n.thumb_url)
    ...
    const r = Ft(n), o = f("span", { class: `sg-attach-icon is-${r}`, "aria-hidden": "true" });
...
function Bt(n, i) {
  ...
  s.addEventListener("click", (h) => {
    (h.target === s || h.target === r) && we();
  }), l.addEventListener("click", (h) => {
    h.stopPropagation(), u(-1);
  }), d.addEventListener("click", (h) => {
    h.stopPropagation(), u(1);
  }), document.addEventListener("keydown", p), r.append(l, o, d), s.append(r, a), document.body.appendChild(s), Z = { overlay: s, onKey: p }, c();
}
function we() {
  Z && (document.removeEventListener("keydown", Z.onKey), Z.overlay.remove(), Z = null);
}
let ue = null;
function Ns(n, i, { onUpload: e }) {
...
function Ke(n, i, e) {
  ...
  function p() {
    ...
    u.replaceChildren(), b.forEach((v) => u.append($s(v, n, i, a, t))), c.firstChild.textContent = b.length === 1 ? "1 attachment" : `${b.length} attachments`;
  }
  ...
  g.style.display = "none", h.append(g), g.addEventListener("change", async () => {
    g.files?.length && (await pe(n, i, Array.from(g.files), o), g.value = "", p());
    ...
    b.dataTransfer?.files?.length && (b.preventDefault(), h.classList.remove("is-drop-target"), await pe(n, i, Array.from(b.dataTransfer.files), o), p());
  });
  function _(b) {
    const v = Array.from(b.clipboardData?.files || []);
    v.length !== 0 && (b.preventDefault(), pe(n, i, v, o).then(p));
  }
  ...
  document.addEventListener("keydown", y), setTimeout(() => document.addEventListener("mousedown", S), 0), d.append(c, u, h), document.body.appendChild(d), Ne(d, n), h.focus(), ue = { pop: d, onKey: y, onDocClick: S, anchor: n };
}
function oe() {
  if (!ue) return;
...
function $s(n, i, e, t, s) {
  ...
  if (z(n) && n.thumb_url)
    ...
    const d = Ft(n), c = f("span", { class: `sg-attach-icon is-${d}`, "aria-hidden": "true" });
  ...
  return l.innerHTML = ke, l.addEventListener("click", async (d) => {
    d.stopPropagation(), await Is(i, e, n, t);
  }), r.append(o, a, l), r;
}
function Ne(n, i) {
...
const Vs = ["NSW", "VIC", "QLD", "WA", "SA", "TAS", "ACT", "NT"], Gt = {
  NSW: "New South Wales",
  VIC: "Victoria",
  QLD: "Queensland",
...
function Ot({ editable: n = !0, empty: i = "" } = {}) {
  return (e) => {
    const { value: t, td: s } = e, r = Fs(t);
    ...
    n && s && !s._sgAddressEditBound && (s._sgAddressEditBound = !0, s.addEventListener("dblclick", (d) => {
      d._sgAddressHandled || (d._sgAddressHandled = !0, d.stopPropagation(), Bs(s, e));
    }));
    const o = f("div", {
...
function Bs(n, i) {
  ...
  document.addEventListener("keydown", C), setTimeout(() => document.addEventListener("mousedown", E), 0), document.body.appendChild(t), Ne(t, n), p(), a.input.focus(), a.input.select(), he = { pop: t, onKey: C, onDocClick: E };
}
function Q() {
  if (!he) return;
  const { pop: n, onKey: i, onDocClick: e } = he;
  document.removeEventListener("keydown", i), document.removeEventListener("mousedown", e), n.remove(), he = null;
}
function Hs(n, i, e) {
...
function zt({ color: n = "green", showValue: i = !1 } = {}) {
  return ({ value: e }) => {
    let t = Number(e);
...
function jt({ max: n = 5, precision: i = 0.5 } = {}) {
  const e = i > 0 ? 1 / i : 2;
  return ({ value: t }) => {
    let s = parseFloat(t);
...
function Kt({ showCode: n = !0, fallback: i = null } = {}) {
  return ({ value: e }) => {
    if (k(e)) return "";
...
function qs(n = {}, i = null, e = {}) {
  ...
  return ({ value: a }) => {
    if (k(a)) return "";
    const l = String(a).toLowerCase(), d = r[l] || s, c = t ? js(a) : String(a), u = f("span", { class: `sg-pill sg-pill-${d}` });
    if (i) {
      const p = o[l], h = p ? zs[p] || p : null;
      if (h) {
...
function Xt({
  ...
  return (e) => {
    ...
    return t == null || t === "" ? d.indeterminate = !0 : d.checked = n(t), d.addEventListener("click", (c) => c.stopPropagation()), d.addEventListener("change", (c) => {
      ...
      const u = d.checked, p = s && r?.field != null ? s[r.field] : null;
...
const Ks = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M464 256A208 208 0 1 0 48 256a208 208 0 1 0 416 0zM0 256a256 256 0 1 1 512 0A256 256 0 1 1 0 256zM188.3 147.1c7.6-4.2 16.8-4.1 24.3 .5l144 88c7.1 4.4 11.5 12.1 11.5 20.5s-4.4 16.1-11.5 20.5l-144 88c-7.4 4.5-16.7 4.7-24.3 .5s-12.3-12.2-12.3-20.9V168c0-8.7 4.7-16.7 12.3-20.9z"/></svg>', Ce = '<svg viewBox="0 0 384 512" aria-hidden="true"><path fill="currentColor" d="M73 39c-14.8-9.1-33.4-9.4-48.5-.9S0 62.6 0 80V432c0 17.4 9.4 33.4 24.5 41.9s33.7 8.1 48.5-.9L361 297c14.3-8.7 23-24.2 23-41s-8.7-32.2-23-41L73 39z"/></svg>', Us = '<svg viewBox="0 0 320 512" aria-hidden="true"><path fill="currentColor" d="M48 64C21.5 64 0 85.5 0 112V400c0 26.5 21.5 48 48 48H80c26.5 0 48-21.5 48-48V112c0-26.5-21.5-48-48-48H48zm192 0c-26.5 0-48 21.5-48 48V400c0 26.5 21.5 48 48 48h32c26.5 0 48-21.5 48-48V112c0-26.5-21.5-48-48-48H240z"/></svg>', Ws = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M125.7 160H176c17.7 0 32 14.3 32 32s-14.3 32-32 32H48c-17.7 0-32-14.3-32-32V64c0-17.7 14.3-32 32-32s32 14.3 32 32v51.2L97.6 97.6c87.5-87.5 229.3-87.5 316.8 0s87.5 229.3 0 316.8s-229.3 87.5-316.8 0c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0c62.5 62.5 163.8 62.5 226.3 0s62.5-163.8 0-226.3s-163.8-62.5-226.3 0L125.7 160z"/></svg>', Xs = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M386.3 160H336c-17.7 0-32 14.3-32 32s14.3 32 32 32H464c17.7 0 32-14.3 32-32V64c0-17.7-14.3-32-32-32s-32 14.3-32 32v51.2L414.4 97.6c-87.5-87.5-229.3-87.5-316.8 0s-87.5 229.3 0 316.8s229.3 87.5 316.8 0c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0c-62.5 62.5-163.8 62.5-226.3 0s-62.5-163.8 0-226.3s163.8-62.5 226.3 0L386.3 160z"/></svg>', Ys = ke;
function Yt(n) {
  if (n == null || n === "") return null;
  if (typeof n == "string") {
    const e = n.trim();
...
function Qt({
  ...
  return (r) => {
    ...
    if (c.innerHTML = Ks, c.addEventListener("click", (u) => {
...
function Qs(n, { preferHowler: i } = {}) {
  return i && typeof window < "u" && window.Howl ? new Js(n) : new Zs(n);
}
class Zs {
  constructor(i) {
...
function We(n, i) {
  ae();
  const e = n._sgAudio || Yt(i.value);
  if (!e) return;
  const t = n._sgAudioOpts || { preferHowler: !0, skipSeconds: 10 }, s = Qs(e.url, t), r = f("div", { class: "sg-audio-player", role: "dialog", "aria-label": "Audio player" });
  r.addEventListener("mousedown", (D) => D.stopPropagation());
  ...
  c.innerHTML = Ys, c.addEventListener("click", ae), o.append(a, l, c);
  const u = f("div", {
  ...
  w.innerHTML = Xs, S.append(b, v, w), r.append(o, u, g, S);
  let x = e.duration ?? 0, L = !1, T = null;
  function C(D) {
    const P = Math.max(0, Math.min(100, D));
    p.style.width = P + "%", h.style.left = P + "%";
  }
  function E() {
    const D = s.seek(), O = s.duration() || 0 || x || 0;
    if (O > 0 && O !== x && (x = O, y.textContent = ee(x), u.setAttribute("aria-valuemax", String(Math.floor(x)))), !L) {
      const H = x > 0 ? D / x * 100 : 0;
      C(H), _.textContent = ee(D), u.setAttribute("aria-valuenow", String(Math.floor(D)));
    }
  }
  function R() {
    E(), s.isPlaying() ? T = requestAnimationFrame(R) : T = null;
  }
  function $() {
    T == null && (T = requestAnimationFrame(R));
  }
  function F() {
  ...
  const K = () => {
    ...
    v.dataset.state = "playing", v.innerHTML = Us, v.setAttribute("aria-label", "Pause"), $();
  }, ne = () => {
    v.dataset.state = "paused", v.innerHTML = Ce, v.setAttribute("aria-label", "Play"), F(), E();
  }, se = () => {
    v.dataset.state = "paused", v.innerHTML = Ce, v.setAttribute("aria-label", "Play"), F(), s.seek(0), E();
  };
  s.on("load", K), s.on("play", U), s.on("pause", ne), s.on("end", se), v.addEventListener("click", (D) => {
    D.stopPropagation(), s.isPlaying() ? s.pause() : s.play();
  }), b.addEventListener("click", (D) => {
    D.stopPropagation(), s.seek(Math.max(0, s.seek() - t.skipSeconds)), E();
  }), w.addEventListener("click", (D) => {
    D.stopPropagation();
    const P = s.duration();
    s.seek(Math.min(P || 1 / 0, s.seek() + t.skipSeconds)), E();
  });
  function ie(D) {
    const P = u.getBoundingClientRect(), O = (D.clientX ?? 0) - P.left, H = Math.max(0, Math.min(1, O / P.width)), Fe = s.duration() || x;
    if (!Fe) return;
    const Pe = H * Fe;
    s.seek(Pe), C(H * 100), _.textContent = ee(Pe);
  }
  u.addEventListener("pointerdown", (D) => {
    D.preventDefault(), L = !0, u.setPointerCapture?.(D.pointerId), u.classList.add("is-dragging"), ie(D);
  ...
  u.addEventListener("pointerup", W), u.addEventListener("pointercancel", W), u.addEventListener("keydown", (D) => {
    ...
    let H = null;
    D.key === "ArrowLeft" ? H = Math.max(0, s.seek() - O) : D.key === "ArrowRight" ? H = Math.min(P, s.seek() + O) : D.key === "Home" ? H = 0 : D.key === "End" && (H = P), H != null && (D.preventDefault(), s.seek(H), E());
  });
  function Y(D) {
    D.key === "Escape" ? (D.preventDefault(), ae()) : (D.key === " " || D.code === "Space") && r.contains(document.activeElement) && (D.preventDefault(), s.isPlaying() ? s.pause() : s.play());
  }
  function Ve(D) {
    !r.contains(D.target) && !n.contains(D.target) && ae();
  }
  document.addEventListener("keydown", Y), setTimeout(() => document.addEventListener("mousedown", Ve), 0), document.body.appendChild(r), Ne(r, n), E(), v.focus(), fe = {
    pop: r,
    backend: s,
    onKey: Y,
    onDocClick: Ve,
...
function Zt({
  ...
  return (e) => {
    ...
    return c.append(f("span", { class: "sg-renderer-switch-thumb", "aria-hidden": "true" })), c.addEventListener("click", (u) => {
      ...
      const p = l ? !0 : !d, h = s && r?.field != null ? s[r.field] : null;
      s && r?.field != null && (s[r.field] = p), o?.applyTransaction && o.applyTransaction({ update: [s] });
      const g = a?.closest('[data-controller~="grid"]');
      g && g.dispatchEvent(new CustomEvent("grid:cellValueChanged", {
        bubbles: !0,
        detail: { rowId: s?.id ?? s?._sg_id, colId: r?.field, oldValue: h, newValue: p }
      }));
    }), c;
  };
}
const ei = /^(https?:\/\/|mailto:)/i;
function _e(n) {
  return String(n).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
}
function Me(n) {
  let i = n;
  return i = i.replace(/`([^`\n]+)`/g, (e, t) => `<code>${t}</code>`), i = i.replace(/\[([^\]\n]+)\]\(([^)\n]+)\)/g, (e, t, s) => ei.test(s) ? `<a href="${s}" target="_blank" rel="noopener noreferrer">${t}</a>` : e), i = i.replace(/\*\*([^*\n]+)\*\*/g, "<strong>$1</strong>"), i = i.replace(/__([^_\n]+)__/g, "<strong>$1</strong>"), i = i.replace(/(^|[\s(])\*([^*\n]+?)\*(?=[\s).,;:!?]|$)/g, "$1<em>$2</em>"), i = i.replace(/(^|[\s(])_([^_\n]+?)_(?=[\s).,;:!?]|$)/g, "$1<em>$2</em>"), i = i.replace(/~~([^~\n]+)~~/g, "<del>$1</del>"), i;
}
function ti(n) {
  const i = n.split(`
`), e = [];
...
function Jt({ inline: n = !1 } = {}) {
  return ({ value: i, td: e }) => {
    if (k(i)) return "";
    const t = _e(i), s = n ? Me(t) : ti(t);
    if (e) {
...
function en({ maxKeys: n = 3, indent: i = 2 } = {}) {
  return ({ value: e, td: t }) => {
    if (e == null || e === "") return "";
    let s = e;
    if (typeof e == "string")
    ...
    a.innerHTML = cs, o.append(a), o.append(f(
      "span",
      { class: "sg-renderer-json-summary-text" },
      document.createTextNode(si(s, n))
    ));
    const l = document.createElement("pre");
    if (l.className = "sg-renderer-json-pre", l.innerHTML = ni(JSON.stringify(s, null, i)), r.append(o, l), o.addEventListener("click", (d) => d.stopPropagation()), t) {
...
function tn({
  ...
  return ({ value: o, row: a }) => {
    ...
    for (const c of l) {
      const u = ii(c, a, n, i);
      d.append(ri(c, a, u, { showThumb: e, href: t, fallback: r }));
    }
    return d;
  };
}
...
function ri(n, i, e, { showThumb: t, href: s, fallback: r }) {
...
function nn({
  ...
  return ({ value: s }) => {
    ...
    for (const a of r) {
      ...
      /^(gray|red|orange|yellow|green|blue|indigo|purple|pink)$/.test(d) ? c.classList.add(`sg-pill-${d}`) : (c.style.background = d, c.style.color = oi(d)), o.append(c);
    }
    return o;
  };
}
...
function rn({
  ...
  return ({ value: t }) => {
    if (k(t)) return "";
    const { from: s, to: r } = li(t), o = (l) => l == null || l === "";
    if (o(s) && o(r)) return "";
    if (o(s))
      return f(
        "span",
...
function an({
  ...
  return ({ value: r }) => {
    ...
    try {
      const d = ts(o);
      a = ds(d, { moduleSize: n, margin: i, background: e, foreground: t });
    } catch {
      return f(
        "span",
  ...
  return ({ value: e, td: t }) => {
    ...
    if (n && r.append(f(
      ...
      a.innerHTML = ge, a.addEventListener("click", async (l) => {
        l.stopPropagation();
        try {
          navigator.clipboard?.writeText ? await navigator.clipboard.writeText(s) : Lt(s), a.innerHTML = St, a.classList.add("is-copied"), setTimeout(() => {
...
const ci = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M225.8 468.2L46.9 295.4C18.9 268.2 0 230.1 0 187.8C0 117.4 56.1 60 125.7 60c25.3 0 51 9.2 70.3 25.4L256 138l60-52.6C335.3 69.2 361 60 386.3 60C455.9 60 512 117.4 512 187.8c0 42.3-18.9 80.4-46.9 107.6L286.2 468.2c-7.2 7.4-17.1 11.8-27.6 11.8c-10.5 0-20.4-4.4-27.6-11.8z"/></svg>', ui = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M313.4 32.9c26 5.2 42.9 30.5 37.7 56.5l-2.3 11.4c-5.3 26.7-15.1 52.1-28.8 75.2H464c26.5 0 48 21.5 48 48c0 18.5-10.5 34.6-25.9 42.6C497 275.4 504 288.9 504 304c0 23.4-16.8 42.9-38.9 47.1c4.4 7.3 6.9 15.8 6.9 24.9c0 21.3-13.9 39.4-33.1 45.6c.7 3.3 1.1 6.8 1.1 10.4c0 26.5-21.5 48-48 48H294.5c-19 0-37.5-5.6-53.3-16.1l-38.5-25.7C176 420.4 160 390.4 160 358.3V320 272 247.1c0-29.2 13.3-56.7 36-75.1c16.3-13.2 28.9-30.4 36.6-50l8.1-20.3c5-12.4 11.3-24.2 19-35.2l4.4-6.3 0 0c1.4-2 1.1-4.7-.6-6.4l0 0c-3.8-3.8-9.9-3.8-13.7 0L208.8 84.9c-3 3-7 4.7-11.3 4.7c-8.8 0-16-7.2-16-16V63.4c0-8.8 7.2-16 16-16h.5c8.8 0 16 7.2 16 16v6.2c34-2.4 71-13 100-25.6c11-4.8 22.1-9.3 33-13.6l1-.4c12-4.6 27.3-9.3 41.4-9.3z"/></svg>', pi = '<svg viewBox="0 0 512 512" aria-hidden="true"><path fill="currentColor" d="M313.4 479.1c26-5.2 42.9-30.5 37.7-56.5l-2.3-11.4c-5.3-26.7-15.1-52.1-28.8-75.2H464c26.5 0 48-21.5 48-48c0-18.5-10.5-34.6-25.9-42.6C497 236.6 504 223.1 504 208c0-23.4-16.8-42.9-38.9-47.1c4.4-7.3 6.9-15.8 6.9-24.9c0-21.3-13.9-39.4-33.1-45.6c.7-3.3 1.1-6.8 1.1-10.4c0-26.5-21.5-48-48-48H294.5c-19 0-37.5 5.6-53.3 16.1L202.7 73.8C176 91.6 160 121.6 160 153.7V192v48 24.9c0 29.2 13.3 56.7 36 75.1c16.3 13.2 28.9 30.4 36.6 50l8.1 20.3c5 12.4 11.3 24.2 19 35.2l4.4 6.3c1.4 2 1.1 4.7-.6 6.4c-3.8 3.8-9.9 3.8-13.7 0L208.8 427.1c-3-3-7-4.7-11.3-4.7c-8.8 0-16 7.2-16 16v10.2c0 8.8 7.2 16 16 16h.5c8.8 0 16-7.2 16-16v-6.2c34 2.4 71 13 100 25.6c11 4.8 22.1 9.3 33 13.6l1 .4c12 4.6 27.3 9.3 41.4 9.3z"/></svg>', xe = ["😞", "😕", "😐", "🙂", "😄"], Ye = {
  star: J,
  heart: ci
}, Qe = {
  star: "#f59e0b",
...
function dn({
  ...
  if (n === "smiley") return hi({ max: i });
  if (n === "thumb") return fi();
  if (n === "nps") return gi();
...
function fi() {
  return ({ value: n }) => {
    ...
    return i > 0 ? (e.classList.add("is-up"), e.title = "Thumbs up", e.innerHTML = ui) : i < 0 ? (e.classList.add("is-down"), e.title = "Thumbs down", e.innerHTML = pi) : (e.classList.add("is-neutral"), e.title = "Neutral", e.append(document.createTextNode("—"))), e;
  };
}
function gi() {
  return ({ value: n }) => {
...
function cn({
  ...
  return ({ value: d }) => {
    ...
    const h = i - n || 1, g = (x) => Math.max(n, Math.min(i, x)), _ = (x) => (g(x) - n) / h * a, y = p && p.length ? p.map(Number) : [n + h * 0.6, n + h * 0.8], S = [n, ...y, i];
    let b = "";
    for (let x = 0; x < S.length - 1; x++) {
      const L = _(S[x]), T = _(S[x + 1]) - L, C = s[x] || s[s.length - 1];
      b += `<rect x="${L.toFixed(2)}" y="0" width="${T.toFixed(2)}" height="${l}" fill="${C}"/>`;
    }
    const v = l * 0.42, w = (l - v) / 2;
    if (b += `<rect x="0" y="${w.toFixed(2)}" width="${_(c).toFixed(2)}" height="${v.toFixed(2)}" fill="${r}"/>`, u != null && Number.isFinite(u)) {
      const x = _(u), L = l * 0.85, T = (l - L) / 2;
      b += `<rect x="${(x - 1).toFixed(2)}" y="${T.toFixed(2)}" width="2" height="${L.toFixed(2)}" fill="${o}"/>`;
    }
...
function un({
  ...
  return ({ value: a }) => {
    ...
    const d = (n - i) / 2, c = n / 2, u = n / 2, p = 2 * Math.PI * d, h = p * (1 - l / 100), g = `<text x="${c}" y="${u + 0.5}" text-anchor="middle" dominant-baseline="middle" font-size="${(n * 0.32).toFixed(1)}" font-weight="600" fill="currentColor">${Math.round(l)}</text>`, _ = `<svg class="sg-renderer-donut" viewBox="0 0 ${n} ${n}" width="${n}" height="${n}" aria-hidden="true"><circle cx="${c}" cy="${u}" r="${d}" fill="none" stroke="${t}" stroke-width="${i}"/><circle cx="${c}" cy="${u}" r="${d}" fill="none" stroke="${o}" stroke-width="${i}" stroke-dasharray="${p.toFixed(2)}" stroke-dashoffset="${h.toFixed(2)}" stroke-linecap="round" transform="rotate(-90 ${c} ${u})"/>` + (s && !r ? g : "") + "</svg>";
...
function pn({
  ...
  return ({ value: l, td: d }) => {
    ...
    const h = Math.max(...p, 1), g = p.reduce((C, E) => C + E, 0), _ = u && u.length ? 10 : 0, y = 1, S = 1, b = n - y * 2, v = i - S * 2 - _, w = Math.max(1, (b - (p.length - 1) * s) / p.length);
    let x = "";
    for (let C = 0; C < p.length; C++) {
      const E = p[C], R = E / h * v, $ = y + C * (w + s), F = S + v - R, K = t ? E === h ? 1 : 0.45 : 0.85, U = u && u[C] != null ? `${u[C]}: ${E}` : `Bin ${C + 1}: ${E}`;
    ...
    const T = `<svg class="sg-renderer-histogram" viewBox="0 0 ${n} ${i}" width="${n}" height="${i}" preserveAspectRatio="none" aria-hidden="true">` + x + L + "</svg>";
...
const _i = {
  ...
  promoter: "green",
  safe: "green"
}, bi = { red: "#ef4444", amber: "#f59e0b", green: "#10b981" };
function hn({
  size: n = 10,
  thresholds: i = null,
  inverted: e = !1,
  ...
  return ({ value: s }) => {
    if (k(s)) return "";
    let r;
    if (i && Number.isFinite(Number(s))) {
      const a = Number(s), l = e ? i[1] : i[0], d = e ? i[0] : i[1];
      e ? r = a >= l ? "red" : a >= d ? "amber" : "green" : r = a <= l ? "red" : a <= d ? "amber" : "green";
    } else if (r = _i[String(s).toLowerCase()] || null, !r) return "";
    const o = f("span", {
    ...
    return o.append(f("span", {
      ...
      style: `width:${n}px; height:${n}px; background:${bi[r]};`,
      "aria-label": r
    })), t && o.append(f(
      "span",
...
function fn({
  ...
  return ({ value: t, td: s }) => {
    ...
    if (Number.isFinite(Number(t)))
      r = Math.max(0, Math.min(n.length - 1, Math.floor(Number(t))));
    else {
      const a = String(t).toLowerCase();
      r = n.findIndex((l) => String(l).toLowerCase() === a);
    }
    if (r < 0) return "";
    const o = f("div", {
      ...
      style: `--ts-color: ${i};`,
      role: "list",
      "aria-label": `Step ${r + 1} of ${n.length}: ${n[r]}`
    });
...
function gn({
  ...
  return ({ value: e }) => {
    if (k(e)) return "";
    const t = String(e), s = f("span", { class: "sg-renderer-mentions" }), r = t.split(yi);
    for (const o of r)
      if (o)
        if (o[0] === "@") {
          const a = o.slice(1), l = typeof n == "function" ? n(a) : null;
          s.append(Ze(o, l, "sg-renderer-mention"));
        } else if (o[0] === "#") {
          const a = o.slice(1), l = typeof i == "function" ? i(a) : null;
          s.append(Ze(o, l, "sg-renderer-hashtag"));
        } else
...
function Ze(n, i, e) {
  ...
  return i && t.addEventListener("click", (s) => s.stopPropagation()), t.append(document.createTextNode(n)), t;
}
function mn({
  chars: n = null,
  lines: i = null,
...
function _n({
  ...
  const r = { style: "unit", unit: n, unitDisplay: i, ...s };
  e != null && (r.minimumFractionDigits = e, r.maximumFractionDigits = e);
  let o;
  try {
    o = new Intl.NumberFormat(t, r);
  } catch {
...
const vi = /^((25[0-5]|2[0-4]\d|[01]?\d?\d)\.){3}(25[0-5]|2[0-4]\d|[01]?\d?\d)$/, wi = /^(([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|:(:[0-9a-fA-F]{1,4}){1,7}|::)$/;
function Ci(n) {
  return vi.test(n);
}
function xi(n) {
...
function bn({
  ...
  return ({ value: i, row: e }) => {
    if (k(i)) return "";
    const t = String(i).trim(), s = Ci(t), r = !s && xi(t);
    if (!s && !r)
      return f("span", {
    ...
    const o = f("span", {
      class: `sg-renderer-ip ${r ? "is-v6" : "is-v4"}`,
      title: s ? "IPv4" : "IPv6"
    });
    if (n && e?.[n]) {
...
function yn({
  ...
  return ({ value: e }) => {
    ...
    const r = `${s.slice(0, 3)}-${s.slice(3)}`, o = s.slice(0, 2), a = n[o], l = f("span", { class: "sg-renderer-bsb" });
    return l.append(f(
      "span",
...
function Ai(n) {
  const i = String(n).replace(/\s+/g, "");
  if (i.length !== 9 || !/^\d{9}$/.test(i)) return !1;
  const e = [8, 7, 6, 5, 4, 3, 2, 1];
  let t = 0;
  for (let s = 0; s < 8; s++) t += parseInt(i[s], 10) * e[s];
  return parseInt(i[8], 10) === (10 - t % 10) % 10;
}
function Li(n) {
  const i = String(n).replace(/\D/g, "");
  return i.length !== 9 ? String(n) : `${i.slice(0, 3)} ${i.slice(3, 6)} ${i.slice(6)}`;
}
function vn() {
  return ({ value: n }) => {
    if (k(n)) return "";
    ...
    return f("a", {
      class: "sg-renderer-link sg-renderer-mono",
      href: `https://abr.business.gov.au/Search/ResultsActiveASIC?SearchText=${i}`,
      target: "_blank",
      rel: "noopener noreferrer",
      title: "Look up on ABR"
    }, document.createTextNode(Li(n)));
  };
}
function wn() {
  return ({ value: n, td: i }) => {
    if (i && i.classList.add("sg-renderer-mask-numeric"), k(n)) return "";
    ...
    const s = t.slice(-3), r = t.length - 3, o = "•".repeat(r);
    return t.length === 9 ? `${o.slice(0, 3)} ${o.slice(3)} ${s}` : `${o.slice(0, 2)} ${o.slice(2)} ${s}`;
  };
}
function Mi(n) {
  if (n.length !== 10 || !/^[2-6]\d{9}$/.test(n)) return !1;
  const i = [1, 3, 7, 9, 1, 3, 7, 9];
  let e = 0;
  for (let t = 0; t < 8; t++) e += parseInt(n[t], 10) * i[t];
  return e % 10 === parseInt(n[8], 10);
}
function Cn() {
  return ({ value: n }) => {
    if (k(n)) return "";
    const i = String(n).trim().replace(/\s+/g, ""), e = /^(\d{10})(?:[\/-]?(\d))?$/.exec(i);
    if (!e || !Mi(e[1]))
      return f("span", {
        class: "sg-renderer-invalid",
        title: e ? "Invalid Medicare (checksum failed)" : "Invalid Medicare format"
      }, document.createTextNode(String(n)));
    const t = e[1], s = e[2], r = `${t.slice(0, 4)} ${t.slice(4, 9)} ${t.slice(9)}` + (s ? ` / ${s}` : "");
    return f(
      "span",
...
`);
  ...
  connect() {
    ...
    }, this._captureInitialMarkup(), this._buildChrome(), this.element.gridApi = Xn(this), queueMicrotask(() => this._initialLoad());
  }
  disconnect() {
  ...
  _openFallbackFilterPopover(e, t) {
    const s = this.state.filterModel[e.field] || {}, r = $i(e.filter), o = m("div", { class: "sg-filter-popover" }), a = m("select");
  ...
  registerColumn(e, t) {
    ...
    if (s >= 0) {
      ...
      if (d._headerEl === t && Ni(d, l)) return;
  ...
  toggleSort(e, t = !1) {
    ...
    let r;
    s === -1 ? r = { colId: e, sort: "asc" } : this.state.sortModel[s].sort === "asc" ? r = { colId: e, sort: "desc" } : r = null, t ? (s >= 0 && this.state.sortModel.splice(s, 1), r && this.state.sortModel.push(r)) : this.state.sortModel = r ? [r] : [], this.scheduleRender("sort"), M(this.element, "grid:sortChanged", { sortModel: this.state.sortModel });
  }
  setSortModel(e) {
  ...
  filteredCount() {
    ...
    return s = it(s, this.state.quickFilter, t), s.length;
  }
  ...
  startEditingCell(e, t, s = void 0) {
    ...
    o && (this.state.editing = { rowId: e, colId: t, originalValue: I(o, r), initialValue: s }, this.scheduleRender("cells"));
  }
  stopEditing(e = !1) {
    if (!this.state.editing) return;
    const { rowId: t, colId: s, originalValue: r, draftValue: o } = this.state.editing, a = this._tbody.querySelector(`tr[data-row-id="${X(t)}"] td[data-col-id="${X(s)}"]`);
    let l = r;
    if (!e && a) {
      const d = a.querySelector("[data-editor-input]") || a.querySelector("input,select,textarea");
      d ? l = Ii(d.value, this._colByField(s)?.type) : o !== void 0 && (l = o);
    }
  ...
  setColumnPinned(e, t) {
    ...
    if (!s) return;
    const r = t || null;
    s.pinned = r, this._runtimeOverrides[e] = { ...this._runtimeOverrides[e] || {}, pinned: r }, this._reorderForPinning(), this.scheduleRender("columns"), M(this.element, "grid:columnPinned", { colId: e, pinned: r });
  }
  setColumnWidth(e, t) {
    const s = this._colByField(e);
    if (!s) return;
    const r = Math.max(s.minWidth || 40, Math.min(s.maxWidth || 4e3, t));
    s.width = r, this._runtimeOverrides[e] = { ...this._runtimeOverrides[e] || {}, width: r }, this.scheduleRender("columns"), M(this.element, "grid:columnResized", { colId: e, width: r });
  }
  moveColumn(e, t) {
    const s = this.state.columnDefs.findIndex((o) => o.field === e);
    if (s < 0 || s === t) return;
    const [r] = this.state.columnDefs.splice(s, 1);
    this.state.columnDefs.splice(t, 0, r), this.scheduleRender("columns"), M(this.element, "grid:columnMoved", { colId: e, fromIndex: s, toIndex: t });
  }
  autoSizeColumn(e) {
    const t = this._colByField(e);
    if (!t) return;
    const s = X(e), r = this._thead?.querySelector(
  ...
  applyTransaction(e) {
    ...
    }), this.state.rowData = Array.from(o.values()), this.scheduleRender("data"), M(this.element, "grid:rowDataChanged", { rows: this.state.rowData }), { added: t, updated: s, removed: r };
  }
  setColumnDefs(e) {
...
`);
  ...
  _render() {
    const e = this._dirty;
    this._dirty = /* @__PURE__ */ new Set(), (e.has("data") || e.has("filter") || e.has("sort") || e.has("page") || e.has("group") || e.has("pivot") || e.has("tree") || e.size === 0) && (this._displayList = Un({
  ...
  _renderHeader() {
    ...
    const e = this._visibleCols(), t = qn(e, this._headerLayoutOpts());
  ...
  _ensureHeaderChrome(e, t, s) {
    ...
    t.filter ? a || (a = m("span", {
      ...
      title: "Filter"
    }), a.innerHTML = Ti, r.appendChild(a)) : a && a.remove(), t.resizable !== !1 && !e.querySelector(".sg-resize-handle") && !t._isCheckbox && e.appendChild(m("span", {
  ...
  _renderBody() {
    ...
    let r = t, o = 0;
    if (s) {
      const p = this._viewport?.clientHeight || 400, h = this.state.rowHeight, g = Wn(this.state.scrollTop, p, h, t.length, 8);
  ...
  _renderRow(e, t, s, r) {
    ...
    for (let g = 0; g < s.length; g++) {
      ...
      const y = _._isRowNumber || _._isCheckbox || _._isGroupCol || _._isMasterExpand, S = p && !y ? Number(p[_.field]) : 0, b = Math.max(1, Math.min(S || 1, s.length - g));
      b > 1 && (h = b - 1);
      ...
      if (_._isMasterExpand) {
        ...
        const L = this._isDetailExpanded(this._rowId(t)), T = m("span", {
      ...
      if (this.state.editing && this.state.editing.rowId === this._rowId(t) && this.state.editing.colId === _.field) {
        ...
        const L = this.state.editing.initialValue !== void 0 ? this.state.editing.initialValue : I(t, _), { node: T, control: C } = this._buildEditor(_, L);
        w.appendChild(T);
        const E = this.state.editing.initialValue !== void 0;
        queueMicrotask(() => {
          if (C?.focus(), E || C?.select?.(), C?.type && ki.has(C.type))
            try {
              C.showPicker?.();
            } catch {
            }
  ...
  getRangeAggregates() {
    return this.state.cellSel.ranges.length ? Vn(this._cellRangeRawValues()) : null;
  }
  ...
  _rangeRect(e) {
    if (!e) return null;
    const t = this._displayList.pageRows, s = this._visibleCols(), r = (u) => t.findIndex((p) => this._rowId(p) === u), o = (u) => s.findIndex((p) => p.field === u), a = r(e.anchor.rowId), l = o(e.anchor.colId);
  ...
  _tabToEditableCell(e) {
    ...
    if (!t) return;
    const s = this._visibleCols().filter((p) => p.editable && !p._isCheckbox), r = this._displayList.pageRows, o = r.findIndex((p) => this._rowId(p) === t.rowId), a = s.findIndex((p) => p.field === t.colId);
  ...
  toggleTreeRow(e) {
    ...
    M(this.element, s ? "grid:treeRowCollapsed" : "grid:treeRowExpanded", { rowId: e, row: r });
  }
  expandTreeRow(e) {
    if (!this.state.tree?.enabled) return;
    const t = String(e);
...
class Ie extends q {
  ...
  connect() {
    if (this.grid = Yn(this.element, "grid", this.application), !!this.grid) {
...
class Ln extends q {
  ...
  _build() {
    ...
    this._columnsTab = m("button", {
      ...
      "aria-pressed": "true",
      title: "Columns"
    }), this._columnsTab.innerHTML = Vi, this._columnsTab.addEventListener("click", () => this._onTabClick("columns")), i.appendChild(this._columnsTab), this.element.append(this._content, i);
  }
  _onTabClick(i) {
  ...
  _renderColumnsList() {
    ...
    for (const a of this._columns()) {
      ...
      s.has(a.field) && p.appendChild(m("span", { class: "sg-tag sg-tag-group", title: "Row group" }, "group")), r.has(a.field) && p.appendChild(m("span", { class: "sg-tag sg-tag-pivot", title: "Pivot column" }, "pivot")), o.has(a.field) && p.appendChild(m("span", { class: "sg-tag sg-tag-value", title: `Value (${o.get(a.field)})` }, o.get(a.field))), l.append(d, c, u, p), this._wireDragSource(l, a.field), t.appendChild(l);
    }
...
function Pi(n) {
  const i = n ?? Rn.start();
  return i.register("grid", $e), i.register("header-cell", Ie), i.register("row", xn), i.register("cell", Sn), i.register("filter", An), i.register("pagination", be), i.register("side-panel", Ln), i;
}
const Bi = {
  start: Pi,
  GridController: $e,
  ...
  SidePanelController: Ln,
  registerRenderer: A,
  getRenderer: lt,
  listRenderers: us,
  renderers: Ei
};
typeof window < "u" && !window.__stimulusGridStarted && (window.__stimulusGridStarted = !0, window.StimulusGrid = Bi);
export {
  Sn as CellController,
  An as FilterController,
  $e as GridController,
  Ie as HeaderCellController,
  be as PaginationController,
  xn as RowController,
  Ln as SidePanelController,
  Bi as default,
  lt as getRenderer,
  us as listRenderers,
